...
日期：2026-09-17。审查代码基线：`2bd5b19`。历史 P0–P8 与 v2 embedding 已实施；2026-09-18 的 KQ04 v3 全量重建另按 R0–R7 推进。当前 **R0 仅完成路由基线与只读失败诊断的一部分**，事实答案人工 gold、Docling、v3 数据层、双向量、生产检索、UI 与激活均未完成。不得继续沿用“全部尚未实施”，也不得把旧 v2 激活写成 v3 已投入查询。

2026-09-18 R0 增量：冻结 300 条分层合同（250 基础、50 边界；50 个真实目录型号种子），同一产品对不跨 development/validation/holdout；“WR3000和WR6500H的区别”仅占一条。离线评测现执行知识动作、实体和属性解析，而非只统计顶层 knowledge 路由。250/250 基础动作与实体通过，全部实体解析 300/300；边界动作 20/50，剩余差异保留为后续修复证据。答案与来源坐标人工复核仍为 0，不能称 300 条真实答案 gold 已完成。只读真实数据库诊断确认该问题已识别为 `compare-facts/category-default`，但事实、旧词法和活动结构化词法候选均为 0；两路 dense 在 R0 按零付费规则未运行。
...
- 本文件最初是审查/计划；后续用户已明确授权 P0–P8 分阶段实施。实际完成状态、证据与仍未满足的线上边界以第9节阶段表和各阶段证据为准，不能仅凭计划文字宣称通过。
