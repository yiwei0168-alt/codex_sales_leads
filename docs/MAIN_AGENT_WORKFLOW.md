...
Coverage continuation: the registry now exposes 94 tools. The latest shared-service adapters cover workspace mode, feedback regeneration, mailbox screening/learning/lifecycle, mailbox connection shutdown/deletion, stale-operation reconciliation, administrator Gold review, approved mailbox knowledge, exact-approved shared-text knowledge publication, hash-verified shared binary registration, persisted contact-enrichment status, staged contact verification and formal standalone score publication. Credentials never enter tool inputs, and publish/destructive effects retain central approval. [The capability matrix](MAIN_AGENT_CAPABILITY_COVERAGE_MA11.md) lists the remaining acceptance gaps and separates later MCP/browser/private-Git work.
