...
2026-09-24 / MA24 真实上传探针：`knowledge:verify-tree-real-uploads-local` 在本地用 PDF/PPTX/XLSX 各一份创建临时私有上传作业，运行现有解析器并逐份注册，核对可检索状态、坐标、幂等、跨账户拒绝和撤销禁引；结束后清理临时副本和记录。该脚本直接驱动作业状态，不覆盖生产 worker 调度与重启恢复。
