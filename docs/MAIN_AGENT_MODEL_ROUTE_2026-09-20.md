...
Read-only receipt timing audit: OpenRouter `created_at` to `finalized_at` was **543 seconds** for the first turn and **901 seconds** for the second. Local completion records followed provider finalization by about **257** and **319 seconds**, respectively, because this verifier reconciled the first receipt and polled manually; those gaps are not model processing time. The provider receipt has no separate queue-start or inference-start timestamp, so its 543/901 seconds cannot be divided reliably into queue versus computation. The two turns were sequential because final synthesis depended on the discovered-tool result. A continuously running worker is designed to poll pending receipts about every 30 seconds, subject to worker availability; that production latency was not measured in this synthetic run.
