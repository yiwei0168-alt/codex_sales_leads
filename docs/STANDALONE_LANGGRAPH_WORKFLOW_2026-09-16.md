...
## KQ01 知识工作流实施状态（2026-09-17）

当前注册图新增 `knowledge_workflow`。助手通过轻量意图后进入同一个可展开知识子图；知识页通过 SDK 调用独立根图，不再在 Next.js 内直调回答 runner。详见[通用知识优化计划](KNOWLEDGE_RETRIEVAL_OPTIMIZATION_PLAN_2026-09-17.md)与[P5证据](KNOWLEDGE_RETRIEVAL_P5_EVIDENCE_2026-09-17.md)。

P0–P8已建立评测、证据校验、受控原件、结构化影子代、事实合同、共享图、混合检索修复、版本绑定缓存/遥测/回退命令及本地发布回归。P3 影子代仍未激活，P7 dry-run 显示 10,871 个向量仍需付费重建且费用未知；P8 的30次真实本地快路径样本达到P95 793.10ms，并在两视口完成知识闭环，但复杂live语义、账单与用户采用仍未知。[P8证据](KNOWLEDGE_RETRIEVAL_P8_RELEASE_2026-09-17.md)。

P5 已把资料定位、实体/版本解析、已验证事实查询和按需生成拆成可展开知识子图，并注册 `knowledge_workflow` 根图。线索图只复用检索证据层，不增加用户意图/回答生成调用。默认保留 C14 的一次轻量意图识别，明确文件/事实在其后本地完成；全本地优先意图仍属于未确认可选项。既有租户边界、费用观察、错误恢复及 LG05 trace 正文隐藏保持。
