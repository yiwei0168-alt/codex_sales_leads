...
当前发布边界保持保守：P3/P4 的 v2 语料代和事实代仍处于 validated/inactive，P7 dry-run 显示激活前仍需生成 10,871 个向量，费用未知，因此本阶段没有付费重嵌入或切换 active generation。离线和本地快路径通过不等于复杂生成语义、真实供应商账单或用户采用已经通过。
...
复杂解释、真实供应商端到端延迟/现金账单、按语言/版本计算的 evidence Recall@8，以及 v2 active generation 的完整线上质量仍未知。下一步应先把 LT700 SIM 等候选口语映射纳入通用注册表和 verified fact 覆盖评测，再在明确预算下生成缺失向量、执行 `knowledge:reindex --apply` 的原子激活，随后按冻结集运行受控 `--live` 抽样；不得用扩大 top-k、跳过证据验证或自动付费重试替代这些门禁。
