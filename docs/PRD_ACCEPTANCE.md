...
Graphiti 本机依赖、Neo4j 连接和本地 LLM/嵌入模型预检通过，验证脚本关闭默认遥测。结构化节点/关系的合成写入、分组回读和清理通过；自由文本 episode 尚未通过实体/关系抽取质量门槛。生产 outbox 投影、图谱结果回库校验与停机回退均未接入或验收；不得把本地探针记为图谱投产。
