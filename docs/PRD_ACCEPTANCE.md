...
图谱候选只返回观察 ID，内部检索再次按 PostgreSQL 的账户、双时间、范围、失效与原文检查，业务起始时间缺失的结果显式标为 `unknown`。合成跨账户候选隔离、图谱不可用及陈旧候选回退聚焦测试通过；主 Agent 接线、真实投影重放和停机端到端尚未通过。未知业务生效时间的观察不被臆定为当前有效。
