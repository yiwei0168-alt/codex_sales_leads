...
记忆观察采用 PostgreSQL 不可覆写记录，并同时保存业务有效时间与系统记录时间。可靠来源的结构化观察使用幂等键，保存来源收据；同范围、同主题的矛盾内容进入冲突待处理，用户可见新增通知，并可用追加失效观察撤销。自动业务事实仅供内部工作，正式评分和对外事实仍需既有核验。迁移 109 和服务层已实现记录、冲突、通知及撤销底座；任务自动抽取、完整界面和真实流程验收待完成。Neo4j 已本机部署，但 Graphiti 投影和本地模型契约尚未通过，不得将图谱当作权威来源。
...
本地抽取 worker 只连接回环地址的 `qwen3:8b`，使用 JSON Schema、精确来源引文、租约及幂等键；部署开关默认关闭，真实模型测试通过前只保留任务队列。当前自动抽取范围仅为明确的用户偏好，经验和业务事实仍待独立验证。
