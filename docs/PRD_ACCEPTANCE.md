...
迁移 113 和本地 worker 已接通 outbox 收据、账户 RLS 读取、租约与延期重试；结构化观察通过 Graphiti 本地节点/关系接口投影，稳定 UUID 使重复投递幂等。合成投影、Node/Python 调用及本机数据库回滚探针通过。`ENABLE_MEMORY_GRAPH_PROJECTION=0` 仍保持生产消费关闭；真实 outbox 恢复、图谱候选回 PostgreSQL 时间/权限校验及停机回退尚待验收。
