...
2026-09-15 stage 243 / A29 decision checkpoint: the user explicitly allows all paid calls during the current product stage. Read-only diagnosis found that the observed block is the duplicate guard for a prior Terra request with unknown cost and no reusable output, not insufficient USD50 remainder. No cumulative/per-request amount or end time was specified, and replaying that fingerprint may charge twice. The rule is recorded as confirmed but not implemented or accepted while explicit replay scope is being clarified; no budget, tariff, request contract, ledger row or provider call changed.
