...
| A06 P05 恢复 | [Gemini 实际模型与恢复身份](GEMINI_DISCOVERY_MODEL_RECOVERY_IDENTITY_2026-09-14.md)、[不可变恢复关联 SQL](PROCESSING_RECOVERY_APPEND_ONLY_ACL_2026-09-14.md) | 合成请求／检查点／隔离 SQL；0 付费 | 空白主覆盖值下备用模型变化会阻止旧付费缓存复用；真实业务中断恢复未全验 | `8c6bf6a`、`1dceea8` |
...
| A09 任务数量与停止 | [O03 当前生产 UI](O03_STAGNATION_STOP_REASON_ACCEPTANCE_2026-09-14.md) | 隔离本地 Chrome；0 付费 | 桌面／手机68项中数量、缺口、停滞新旧口径通过；真实业务结果待 A11 | `013e228` |
| A10 无付费回归 | [当前代码测试与构建](GEMINI_DISCOVERY_MODEL_RECOVERY_IDENTITY_2026-09-14.md)、[最新 UI](O03_STAGNATION_STOP_REASON_ACCEPTANCE_2026-09-14.md) | 本地测试／构建／Chrome；0 付费 | 1,024 项测试、类型检查与构建通过；此前 UI 68 项通过 | `8c6bf6a`、`013e228` |
...
| A13 页面与业务 | [当前 Chrome 双视口](O03_STAGNATION_STOP_REASON_ACCEPTANCE_2026-09-14.md)和矩阵私有知识 SQL | 隔离账号／本地生产 HTTP；0 付费／发信 | 已测页面68项通过；真实跟进、RAG 采用及完整私有链路未全验 | `013e228` |
