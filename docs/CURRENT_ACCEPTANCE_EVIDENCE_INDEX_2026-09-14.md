...
| A05 费率刷新 | [07:49 UTC 官方 FX 重试](FX_SCHEDULED_RECHECK_0749_2026-09-14.md)及现行价证门禁 | 无密钥官方 HTTP／只读状态；0 付费 | 新 FX 未获验证，旧快照过期、付费门禁保持 | `98f1759` |
| A06 P05 恢复 | [不可变恢复关联与预算 SQL](PROCESSING_RECOVERY_APPEND_ONLY_ACL_2026-09-14.md)及矩阵检查点记录 | 隔离 SQL／合成故障；0 付费 | 检查点和历史权限通过；真实业务中断恢复未全验 | `1dceea8` |
...
| A10 无付费回归 | [当前生产构建与 UI](O03_STAGNATION_STOP_REASON_ACCEPTANCE_2026-09-14.md) | 本地测试／构建／Chrome；0 付费 | 1,021项测试、类型检查、构建及68项 UI 通过 | `013e228` |
| A11 最小真实闭环 | [搜索 HTTP 上限预检](A11_DISCOVERY_HTTP_ATTEMPT_CEILING_2026-09-14.md)与矩阵完整费用缺口 | 真实付费仅在整次预检及累计 USD30 门禁放行后 | **未运行、未验收**；整次费用上界仍为 `null` | `c6c4241`（仅预检） |
