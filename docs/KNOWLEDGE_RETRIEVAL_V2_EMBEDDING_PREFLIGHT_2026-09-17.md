...
预算设置后，一个十项批次通过了本地预留门禁，但在 SDK transport 返回前出现 `paid-outcome-unknown`。系统已将该次预留保留为未知结果并禁止相同请求自动重试；没有收到可验证的供应商响应、没有有效向量、没有向量写入、没有 activation、没有 token/API credit/现金金额，也没有重试。因为 transport 中断不能证明请求是否已经到达供应商，所以不得写为“零供应商调用”或将未知费用当作零。仅携带 endpoint 主机名的 TCP 443 连通性检查成功，说明 DNS/TCP 路由可达，但不构成 API 认证、请求体交付或账单成功证据。恢复需要先在百炼控制台核对该时间窗口的请求/账单，再决定是否以新的输入指纹继续；不得直接重放这个十项批次。
