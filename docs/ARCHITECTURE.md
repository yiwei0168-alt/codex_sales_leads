...
Migration 083 provides the inactive v3 storage contract: scoped releases and manifests, source/unit terminal states, parent chunks, row-scoped entities, separate 1,536/1,024-vector columns and partial HNSW indexes, typed facts, review queue, release pointers and a guarded activation function. Every v3 table has forced RLS. The isolated Docling 2.126.0 pipeline and local RapidOCR produced shadow release `rag-v3-shadow-2026-09-18`, covering 281 registered assets, 1,820 units and 3,062 chunks. Qwen and BGE are both complete at 3,062/3,062, but the release remains `building` and has no active pointer. The shared XLSX catalog uses normalized exact model-cell matching so each row binds only its registered entity. Each embedding is bound to a versioned profile, model revision, dimensions and chunk content hash. Activation requires complete per-scope manifests and occurs by a short pointer transaction; a private owner's incomplete corpus cannot block or contaminate another scope. RAGFlow is not a production dependency.

Migration 088 adds the administrator review center contract. Fact decisions remain attached to `knowledge_review_queue_v3`, including reviewer/time/note and optional correction JSON. Human Gold is stored separately with corpus/case hashes, exact source coordinates and revisions. Development and validation must be complete before an explicit retrieval-profile freeze unlocks holdout; activation additionally requires 300/300 reviewed cases and 50/50 holdout. The “知识库 & RAG” page exposes the queues without exposing raw vectors or unrestricted traces. See [R6 review-center acceptance](KNOWLEDGE_RAG_V3_R6_REVIEW_CENTER_2026-09-18.md).
