...
Migration 083 provides the v3 storage contract: scoped releases and manifests, source/unit terminal states, parent chunks, row-scoped entities, separate 1,536/1,024-vector columns and partial HNSW indexes, typed facts, review queue, release pointers and a guarded activation function. Every v3 table has forced RLS. The isolated Docling 2.126.0 pipeline and local RapidOCR produced release `rag-v3-shadow-2026-09-18`, covering 281 registered assets, 1,820 units and 3,062 chunks. Qwen and BGE are both complete at 3,062/3,062; migration 089 and the confirmed fact-quarantine policy allowed this complete release to become `active` on 2026-09-19 with one shared active pointer and zero document/OCR review blockers. The 1,029 open fact reviews remain persisted but are excluded from verified fact reads and the structured fact retrieval lane. The shared XLSX catalog uses normalized exact model-cell matching so each row binds only its registered entity. Each embedding is bound to a versioned profile, model revision, dimensions and chunk content hash. Activation requires complete per-scope manifests and occurs by a short pointer transaction; a private owner's incomplete corpus cannot block or contaminate another scope. RAGFlow is not a production dependency.

Migration 088 adds the administrator review center contract. Fact decisions remain attached to `knowledge_review_queue_v3`, including reviewer/time/note and optional correction JSON. Human Gold is stored separately with corpus/case hashes, exact source coordinates and revisions. Development and validation must be complete before an explicit retrieval-profile freeze unlocks holdout. Migration 088 originally also made 300/300 Gold and 50/50 holdout activation prerequisites; the user-confirmed migration 089 supersedes only that activation condition and instead quarantines every unresolved fact while Gold continues after launch. The “知识库 & RAG” page exposes the queues without exposing raw vectors or unrestricted traces. See [R6 review-center acceptance](KNOWLEDGE_RAG_V3_R6_REVIEW_CENTER_2026-09-18.md) and [R7 isolation activation evidence](KNOWLEDGE_RAG_V3_ACTIVATION_2026-09-19.md).
