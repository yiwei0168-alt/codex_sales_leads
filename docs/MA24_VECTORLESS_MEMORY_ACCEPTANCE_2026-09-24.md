...
2026-09-24 Gold 人工审核入口补充：用户确认亲自在产品界面审核。`/knowledge` 的“复核中心 → Gold 审核”已支持逐题保存答案、活动共享来源哈希、页/幻灯片/工作表、可选原文块 ID 与必填原文短引；同一原件可引用多个位置。无答案且无来源的题目允许以审核备注记录至少 12 字的查证范围，不再诱导填写不存在的来源。旧审核记录保留，但没有原文短引的不计入“精确来源已核”进度；解锁 holdout 需开发集 190 题与验证集 60 题都达到该门槛。活动 v3 之外或已撤销的共享来源不能新保存为 Gold。本机只读审计：development 已保存 11/190、精确来源 0/190；validation 0/60；holdout 0/50。未替用户生成或确认答案，也未解锁 holdout。
2026-09-24 Gold 来源核对补充：审核界面可读取当前共享树的指定页/幻灯片/工作表原文，点击原文块自动填写块 ID 和最多 500 字短引。保存及解锁 holdout 前按来源哈希、单元编号、可选块 ID 复核短引在当前原文块内；无对应块或已停用版本被拒。本机 SQL 探针从活动共享树读取一条样本并由新接口找回同一块；单元测试覆盖错误短引被拒及空白归一后的正确短引可保存。原文一致不代表答案正确，300 题仍由用户逐题判断。
本阶段验证：`tsc --noEmit`、所改 TypeScript 文件 ESLint、13 项 Gold/复核单元测试、Gold 编辑器桌面及移动浏览器 2 项测试、Next.js 生产构建均通过；本机 SQL 探针返回 `matched:true`；重启后 `/knowledge` HTTP 200。尚未产生新的人工 Gold 条目，质量门槛仍待用户审核。
