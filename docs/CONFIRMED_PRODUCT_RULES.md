...
| MA24-05 | Graphiti/Neo4j 在本机部署，PostgreSQL 始终是权威库；图谱由事务 outbox 投影，可延迟/重建，查询必须回库校验。不可用时回退 PostgreSQL。自由文本先用本地 qwen3:8b 做 Schema/真实样本测试，不可用则排队，不静默改云模型。 | outbox、完成任务的本地抽取队列和仅回环地址的租约 worker 已实现；本机 Ollama 0.12.9 与 qwen3:8b 已部署，worker 固定校验模型摘要，启动开关仍关闭。Neo4j 本机容器与 Cypher 探针通过；Graphiti 其余依赖及投影未完成。 | 迁移 112、账户隔离回滚事务、模拟模型测试及真实模型五类合成样本契约通过；仅有 2 条真实任务消息完成只读形状测试，偏好召回质量、图谱回读/回退待验收。 |
