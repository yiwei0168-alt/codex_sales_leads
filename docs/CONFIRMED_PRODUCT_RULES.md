...
### KQ01 — 知识问答与资料访问按通用问题分阶段优化（2026-09-17，用户明确范围与实施授权；P0–P7已实施，P8待验收）
...
已形成[通用知识优化实施交接计划](KNOWLEDGE_RETRIEVAL_OPTIMIZATION_PLAN_2026-09-17.md)。P0–P6已完成评测、证据边界、受控原件、结构化影子代、verified 事实合同、共享知识图和检索修复。P7 新增按用户/过滤器/语料代/别名版/embedding 合同绑定的成功缓存、命中重鉴权、全终态聚合遥测及默认 dry-run 的代切换命令，详见[P7证据](KNOWLEDGE_RETRIEVAL_P7_EVIDENCE_2026-09-17.md)。真实 dry-run 显示 v2 仍需 10,871 个向量且费用未知，因此没有激活或付费重建。历史事实未删除、影子代未激活，不能宣称全库已纠正。计划默认保留 C14 的轻量意图识别；完全本地优先意图的例外尚未确认。P8 及真实性能目标仍须验收。A30/A33/A34及LG02/LG04/LG05继续有效。
