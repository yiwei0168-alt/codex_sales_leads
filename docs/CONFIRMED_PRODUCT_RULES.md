...
> MA24-04、MA24-05 实施/验收状态补充（2026-09-24）：本机 16 条旧活动邮件风格内部学习记忆保留原表、状态及来源后幂等回填为私有双时间观察，业务生效时间未知；16 条观察、通知和 outbox 对照一致，重跑新增 0。真实图谱 outbox 探针修正租户查询和 Windows Python UTF-8 编码后，一条真实观察通过本机投影、账号隔离、PostgreSQL 回校与重放拒绝。图谱默认关闭，余下投影及真实任务召回未验收；历史记忆不自动获得对外表述或正式评分权限。
