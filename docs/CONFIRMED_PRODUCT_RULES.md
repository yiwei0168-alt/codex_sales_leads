...
### KQ01 — 知识问答与资料访问按通用问题审查和规划（2026-09-17，用户明确范围；仅审查/计划完成，产品实施待后续）
...
已形成[通用知识优化实施交接计划](KNOWLEDGE_RETRIEVAL_OPTIMIZATION_PLAN_2026-09-17.md)，包含跨语料只读统计、事实提取/切片反例、P0–P8步骤、文件路径、验收与回退要求；相关[独立编排工作流](STANDALONE_LANGGRAPH_WORKFLOW_2026-09-16.md)已标记为待实施。计划默认保留 C14 的轻量意图识别，仅在识别后为明确资料/事实查询省略不必要的embedding及回答生成；完全本地优先意图的例外尚未确认。计划中的评测数量、性能指标、新表/节点/命令是实施建议，不伪记为用户逐项确认值。A30/A33/A34及LG02/LG04/LG05继续有效。现有58项聚焦回归通过是当前基线证据，不等于本次发现的缺陷已修复或通用语义质量已验收。
