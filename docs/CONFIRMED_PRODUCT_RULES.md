...
### A29 — 当前阶段允许付费调用（2026-09-15，用户明确确认；实施待重放边界澄清）

用户原话：“我在使用产品的时候，付费调用被阻止。当前阶段，允许所有付费调用。”范围是当前产品阶段；用户未给出具体累计美元上限、单请求上限或结束时间。只读诊断确认当前已知阻止是旧 Terra 同指纹请求的 `paid-request-already-recorded`，而不是 USD 50 预算不足；该请求已可能付费，但费用未知且没有可复用输出。“所有”是否显式包含重放这类可能已扣费的未知请求，正向用户澄清。在澄清前未提高或取消预算、未改费率／请求合同、未重放旧请求，也未发生新付费调用。这条确认不能充当实施或测试证据。
