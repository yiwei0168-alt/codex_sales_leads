...
### KQ01 — 知识问答与资料访问按通用问题审查和规划（2026-09-17，用户明确范围；P0基线已实施，P1–P8待后续）

用户先提出简单规格查询应在检索后简单处理、打开 datasheet 应返回原文件链接，并要求审查意图、检索、语义预处理与切片；随后明确：“这里WR3000仅仅是一个例子，审查类似问题并一并生成优化方案。给出实际的计划和步骤，后续我会让成本更优的sol或者Terra模型按照plan优化代码”。确切范围为通用知识请求、原文件访问、不同型号/版本/属性、公司与行业材料，以及共用检索/切片的关联入口；禁止把某个型号的特判当作通用优化完成。当前授权任务是审查和形成后续可执行计划，不是已经实施代码、数据纠错或重新入库。Sol/Terra 指后续实施者模型，不构成产品运行时供应商切换确认。

已形成[通用知识优化实施交接计划](KNOWLEDGE_RETRIEVAL_OPTIMIZATION_PLAN_2026-09-17.md)，包含跨语料只读统计、事实提取/切片反例、P0–P8步骤、文件路径、验收与回退要求；相关[独立编排工作流](STANDALONE_LANGGRAPH_WORKFLOW_2026-09-16.md)已标记为待实施。P0已建立200条确定性离线评测请求、只读语料审计和缺陷基线，详见[P0证据](KNOWLEDGE_RETRIEVAL_P0_BASELINE_2026-09-17.md)；它没有修改知识、向量、事实或产品路由。计划默认保留 C14 的轻量意图识别，仅在识别后为明确资料/事实查询省略不必要的embedding及回答生成；完全本地优先意图的例外尚未确认。计划中的性能指标、新表/节点及P1–P8实现仍是待验收建议，不伪记为用户逐项确认值。A30/A33/A34及LG02/LG04/LG05继续有效。
