...
版本：1.1.0。整理日期：2026-09-12。来源：本项目用户确认记录及仓库文档。
...
- **下一项待确认建议**：选择官方 recipe＋匹配的 V4 tokenizer，固定版本/文件校验值，在隔离离线环境先验证当前两种文本请求格式。通过前仅为离线诊断，不用于付费放行；不改 thinking、不自动跟随最新版、不将样本误差上浮若干百分比冒充数学上界。
