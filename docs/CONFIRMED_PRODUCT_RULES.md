...
> MA24-04、MA24-05 实施/验收状态补充（2026-09-24）：本机 16 条旧活动邮件风格内部学习记忆保留原表、状态及来源后幂等回填为私有双时间观察，业务生效时间未知；16 条观察、通知和 outbox 对照一致，重跑新增 0。真实图谱 outbox 探针修正租户查询和 Windows Python UTF-8 编码后，一条真实观察通过本机投影、账号隔离、PostgreSQL 回校与重放拒绝；临时本机 worker 将 16/16 条投影，待处理 0。图谱默认关闭，真实任务召回未验收；历史记忆不自动获得对外表述或正式评分权限。

> MA24-01 质量门槛实施/验收状态补充（2026-09-24）：用户选择在产品界面亲自审核 300 题 Gold。审核来源现记录活动共享资料哈希、页/幻灯片/工作表、原文短引及可选块 ID/表格行；无答案且无来源须记录查证范围。历史 11 条保留“已保存”，但无短引不计“精确来源已核”；本机审计为 development 11/190 已保存、0/190 精确，validation 0/60，holdout 0/50。开发与验证共 250 题精确审核完成前不能解锁 holdout 或切换生产主路。
> MA24-01 实施补充：Gold 编辑器可读取所选来源页在当前树版本中的原文块并填入块 ID/短引；保存及解锁 holdout 前将短引与活动共享原件的当前树原文按空白归一后逐块核对，页号或块 ID 不匹配则拒绝。这是录入一致性检查，不能替代审核人对答案是否由原文支持的判断。
