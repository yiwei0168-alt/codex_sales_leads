...
事实冲突复核随后按已确认的版本合同修正：冲突只在同实体、同版本、同市场、同属性内比较；集合属性按具体成员比较；蜂窝代际和政策条件登记为集合；设备尺寸与包装尺寸使用独立属性。该通用规则把合法版本差异、多值集合及包装规格移出虚假冲突，事实结果变为 1,953 verified、1,091 candidate、9 conflicting，open fact review 降为 1,029（1,020 low-confidence、9 conflict）。剩余复核项没有自动接受，release 继续保持 `building`、活动指针为 0。
