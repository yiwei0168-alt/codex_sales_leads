...
- `lead_search_provider_call`保存逐调用聚合输入、产出、成本、延迟、重试、fallback和丢弃原因。
- `lead_search_provider_occurrence`保存逐候选来源、首次/辅助发现、轻量门禁、最终角色/资格/分数、展示/选择/采用和分数化贡献。
- `workflow_stage_metric`保存发现阶段原始生成量、新增有效量、下游使用量和聚合成本。
- 当前分数化贡献在同一公司的全部规范化occurrence之间等额分摊；首次发现由`first_discovery`单独统计，不能互相替代。
- 用户后续选择、修改、开发或长期保留尚未全部接入occurrence事件时必须标记`not-observed`，不能把“已展示”当作“已执行”。
