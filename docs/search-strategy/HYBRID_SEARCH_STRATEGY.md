...
## 十四、Brand Owner / Product Company（已确认，显式启用）

Brand Owner / Product Company是以自有品牌销售网络或连接类产品，并掌握产品定义、规格、品牌、定价或产品路线决策权的正式主角色；实际制造可以外包。

- Distributor/Retailer只有少量自有品牌但主营仍为分销/零售时，保持原主角色，自有品牌只作为机会信号。
- ISP拥有定制CPE时仍以ISP为主角色。
- 仅向其他企业提供设计或制造服务的OEM工厂不属于该角色，也不进入产品搜索范围。
- 只有品牌名称但没有产品控制或真实销售能力的商标持有人不通过。
- 成熟直接竞品不会因品牌身份自动成为高价值OEM/ODM客户，合作可行性由统一评分和路径Agent判断。
- 默认不单独搜索。只有用户明确要求品牌公司或启用OEM/ODM客户机会时进入计划；其他轨道发现后可以由角色Agent识别，但不重复搜索、补证或评分。

### Brand Owner搜索和复用

```text
本地候选库/公共证据库
→ SearchAPI Google（自有品牌和产品精确查询）
→ Gemini Full（仅复杂语义或覆盖缺口）
→ Brave / SearchAPI Bing（异构索引缺口）
→ Exa（产品、规格、品牌和技术页面缺口）
→ 统一证据阶段
```

- Brand Owner任务寻找真实控制并销售自有网络产品的公司；OEM/ODM任务寻找可能采购Cudy定制/白牌方案的客户，两种判断不能互相替代。
- 两种任务同时启用时共用候选注册表、抓取内容和一次Flash调用，不执行两套重复搜索或门禁。
- Product Gemini和Places不调用，Tavily仍只在统一证据阶段工作。

### Brand Owner轻量门禁

- `pass`：存在真实自有品牌和相关网络产品，同时有产品定义、品牌经营、销售或市场控制信号。
- `hold`：自有品牌与Distributor/Retailer主业关系不清，可能只是贴牌/授权销售，或品牌、法人和采购主体关系未核实。
- `reject`：只有商标/空壳网站、只销售第三方品牌、纯Marketplace、产品无关，或仅提供制造服务且没有面向市场的自有产品品牌。
- Brand Owner任务不要求OEM/ODM合作开放度；OEM/ODM机会也不能因为候选是Brand Owner就自动通过。
- 模型在同一批调用中返回品牌控制和OEM/ODM机会信号，程序分别生成角色候选状态和机会状态。

### Brand Owner角色评分卡

保持统一100分和既有子权重：

- 产品与应用场景50：产品家族25、客户/场景15、定位兼容10。采用best-enabled-track，不因专注单一SMB/ISP等家族而低于全品类品牌。
- 合作路径与采购影响15：当前路径5、产品定义/采购控制6、选型/市场影响4。
- 同角色规模与覆盖15：相关产品业务规模6、目标市场覆盖5、客户/装机量/销售网络4。只与Brand Owner横向比较，不要求Distributor式下级渠道或Retailer式门店。
- 执行和赋能10：商业/产品运营4、技术/质量/认证/固件/售后3、上市推广和持续经营3。
- 机会与风险10：合作开放度4、战略时机3、竞争与结构风险3。

没有公开OEM历史属于`unknown`，不能直接记0分。成熟竞品、供应链锁定、渠道冲突或完整自主研发制造体系可构成结构性风险，但不能仅凭竞品标签自动判无效。OEM/ODM路径FitScore单独计算，不重复叠加进总分。

## 十五、无泛化Other搜索类别（已确认）

- 不生成“搜索其他类型公司”的任务，也不能以Other给数量目标凑数。
- 暂时无法确定但可能有价值的候选标记为`research-required + role-unresolved`，交给证据与角色Agent。
- 最终确认属于现有体系外的公司必须记录具体角色名称和证据，不能永久归入Other。
- 新角色只有形成稳定业务价值后，才能通过版本化方式增加独立定义、搜索模板、门禁、评分和贡献测量。
- 已确认的“其他合作模式”继续作为合作路径类型保留，与主角色和搜索类别无关。

## 十六、实施状态与结论

当前计划中的候选类别、OEM/ODM机会覆盖、工具边界、实时去重、轻量门禁、数量规则以及搜索到下游的数据流均已逐项确认，并于`main@6c4a1ea`接入生产发现节点。策略配置状态为`active`，数据库迁移`033_hybrid_search_contribution.sql`已应用。

每次搜索工具调用会记录输入、原始/规范化/新增/重复/拒绝输出、credits、模型token、延迟、重试、fallback和丢弃原因；每条候选出现记录会保留首次/辅助发现及门禁结果。统一评分完成后回写最终主角色、eligibility、分数、展示/选择/下游采用和分数化贡献。

本次实现验证使用模拟provider，真实搜索成本、有效产出和下游采用仍为`not-observed`。首次正式运行必须按[贡献测量规范](./SEARCH_CONTRIBUTION_MEASUREMENT.md)发布同口径结果。后续不得从相邻类别机械复制工具组合，也不得增加未版本化的新角色或重复工作流。
