...
- 将确认后的混合搜索策略发布为`1.0.0 active`，替换LangGraph旧Tavily候选发现；Tavily继续只用于统一证据阶段。
- 每轮按route sequence小批次执行，共享一个实时公司注册表；达到质量池目标或同轨连续两批无新增价值时停止后续缺口调用。
- 新候选先做有边界的官网首页直读，再由DeepSeek Flash批量输出最小语义信号；程序生成pass/hold/reject，门禁不可升级Pro，模型不可用时hold。
- 搜索调用、候选occurrence、门禁和最终评分/展示/采用结果写入数据库；GitHub只保存版本、聚合质量和优化结论。
- 实现测试不是成本实测。首次正式搜索前不宣称成本下降，之后按类别比较唯一有效产出、重复浪费、下游采用和单位成本。

状态：已实现并验证；对应`main@6c4a1ea`。
