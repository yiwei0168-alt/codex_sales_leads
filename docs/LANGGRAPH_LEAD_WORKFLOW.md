...
2026-09-17 KQ01 P8：共享知识检索层完成发布回归。线索图继续只复用受租户/范围约束的证据检索，不进入面向用户的文档直开、事实格式化或回答生成分支。离线知识评测、真实SQL语义检查、独立 `knowledge_workflow` 路由和全站隔离UI通过；模型、embedding、搜索、SMTP和付费调用为0。v2语料代保持inactive，复杂live召回/延迟及用户采用未知。[证据](KNOWLEDGE_RETRIEVAL_P8_RELEASE_2026-09-17.md)。

2026-09-17 LG04：独立 Agent Server 的 `lead_workflow` 不再以单个 `run_lead_workflow` 黑盒节点描述生产流程。根图现在公开输入校验、可展开的 `lead_business_flow` 和结果合同；`xray=true` 展开后包含 `retrieve_knowledge`、`build_playbook`、`prepare_recovery_evidence`、`recover_saved_evidence`、`discover_candidates`、`collect_evidence`、`correct_candidates`、`route_candidates`、`score_candidates`、`recover_incomplete_processing`、`review_assessment_anomalies`、`assemble_handoff_briefs` 与 `persist_results`，原条件边和循环保持不变。可视拓扑实例不绑定存储；实际执行实例仍由同一构建器绑定 PostgreSQL `langgraph` schema，继续执行租户、动作、计划身份、暂停、防重放、恢复费用已知性和终态持久化检查。产品 SDK 输入、DTO、数据库和任务完成／失败语义不变。
2026-09-15 stage 246 / A32: `rag-grounded-answer` keeps OpenAI Sol as primary and may issue exactly one separately reserved same-model `amazon-bedrock/us-east-1` request only after sanitized router metadata proves an upstream 401/403/429/5xx. Both wires pin one provider, disable gateway/SDK retries, cap output at 4,096 and preserve A30 public-only redaction. Gateway/account/request errors stop without fallback; a failed fallback has no third attempt. Safe status/provider/request-hash telemetry and SQL/no-network checks pass with zero inference. [Evidence](A32_CONTROLLED_RAG_FALLBACK_STAGE246_2026-09-15.md).
2026-09-15 stage 243 / A29: the owner/rule override reused ten original graph snapshots and emitted two Terra v1 secondary-review attempts, both invalid and fee-unknown. No discovery, evidence, correction or primary score was repeated. Endpoint metadata shows every Terra endpoint omits `temperature`; Terra-only v2 now omits it and uses `max_tokens=8192`, retaining the model, reasoning, schema, data policy and USD11.019202 ceiling. Local/SQL checks pass. The v2 graph resume is held before transport until the user explicitly authorizes sending this task's workspace evidence to OpenRouter. [Evidence](A29_PAID_REPLAY_ADMISSION_STAGE243_2026-09-15.md).
2026-09-14 stage 239 / A28 resume scope: next verify one isolated multi-turn assistant action through the current graph, actual correction/score/review logic, national SQL and production desktop/mobile UI using deterministic external responses. Include required secondary agreement, material-disagreement judge and unresolved-review hold; preserve the stage238 real task and unknown-cost fingerprints untouched. A failed graph's partial metrics and user-facing stop reason need examination before claiming product-level closure. This is a plan, not executed graph or real supplier acceptance. [Resume](RESUME_PRODUCT_ACCEPTANCE_STAGE239_2026-09-14.md).
2026-09-14 stage 238: one new isolated real graph used the S02 Sol v2 playbook successfully, then reached search, evidence, correction and primary scoring. Its required Terra secondary-review HTTP attempt has unknown cost and no valid output; the identical request was not replayed. Nine graph checkpoints remain, but the failed action did not persist a `lead_search_run`, final company, or stage aggregate. Do not call this qualified or complete. [Evidence](S02_MINIMAL_LIVE_PARTIAL_STAGE238_2026-09-14.md).
2026-09-14 stage 237 / S02: the user-approved market-playbook Sol request now serializes `max_tokens=4096` with no `temperature` under a versioned OpenAI-only strict-JSON contract. A new cache identity and exact wire fingerprint separate v2 from the failed v1 request; the unknown v1 reserve remains. Synthetic 4,186-byte product-context wire and no-paid regression pass, while a fresh actual assistant→graph→search→score→save run is still pending. A27 allows documented technical acceptance compromises, not silent changes to the lead qualification standard or USD50 gate. [Evidence](S02_PLAYBOOK_PARAMETER_ACCEPTANCE_STAGE237_2026-09-14.md).
2026-09-14 stage 236: post-FX current-build desktop/mobile UI, task detail, usage, budget and dynamic-reference views passed 68/68 isolated local production checks. No graph node or paid provider ran in this UI regression; the real A26 graph remains stopped at S01 HTTP404, and ten incomplete cost reports retain occupancy. [Evidence](A26_POST_FX_CURRENT_BUILD_UI_STAGE236_2026-09-14.md).
2026-09-14 stage 235: the current acceptance table now reflects the actual graph boundary: two real intent turns, one corrected/confirmed plan and one knowledge lookup, then S01 HTTP404 before discovery. A25 admits only actual bounded calls against A26 USD50 and fixed validation FX; the original unknown S01 reserve remains occupied. This is documentation reconciliation, not graph execution or S02 approval. [Evidence](A26_CURRENT_ACCEPTANCE_TABLE_RECONCILIATION_STAGE235_2026-09-14.md).
2026-09-14 stage 234 / A26: the validation spend scope continues to pin one stored FX version without refresh; ordinary product quote/status expiry now agree on seven days after actual retrieval. The scheduler rechecks duplicate official payloads before the retained snapshot expires. No graph action, paid call or lead output occurred in this regression. Real A11 remains stopped at S01 HTTP404 pending the separate S02 confirmation. [Evidence](A26_WEEKLY_FX_EXPIRY_EDGE_STAGE234_2026-09-14.md).
2026-09-14 stage 233: current-build Chrome desktop/mobile 68/68 local UI and task/usage/budget views passed with isolated synthetic data. The product graph was not newly run; the real A26 action remains failed at existing S01 HTTP404 and its cost reserve is retained. Current USD50 account occupancy is USD23.520920. [Evidence](A26_CURRENT_BUILD_UI_REGRESSION_STAGE233_2026-09-14.md).
2026-09-14 stage 232: actual isolated assistant input2 -> valid plans2 -> corrected Spanish plan1 -> confirmed graph action1 -> knowledge lookup1 -> existing S01 market-plan HTTP404. First graph attempt failed before paid work because the disabled fixture lacked a global workspace; scoped provisioning let the same zero-cost action continue. The current paid S01 request remains recorded and is not automatically replayed; no discovery, evidence, score or persistence node completed. Total occupied USD23.520920/50. S02 is a separate unconfirmed parameter-contract proposal, not a route change already in product. [Evidence](A26_LIVE_PARTIAL_ACCEPTANCE_STAGE232_2026-09-14.md).
2026-09-14 stage 231 / A26: an isolated validation invocation inherits one pinned stored official ECB snapshot through assistant and lead-graph spend scopes, with no age expiry or refresh. Ordinary product refresh is weekly and the current FX validity is seven days. The validation account cap is USD50, original occupancy USD12.324404 retained; A25 per-request admission and unknown-cost stops remain. Read-only quotes for required Kimi/embedding/playbook calls now pass, while unbounded conditional discovery contracts can still pause the graph. No real A11 graph ran. [Evidence](A26_WEEKLY_FX_AND_USD50_LOCAL_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 230: the due official ECB refresh remained stale, so the initial CNY-priced intent and knowledge-embedding steps still pause under A25's per-request FX gate. The missing whole-task forecast no longer independently blocks entry, but this actual first-step hold prevents a real graph run. No paid call or task claim occurred. [Evidence](FX_A25_DUE_RECHECK_STAGE230_2026-09-14.md).
2026-09-14 stage 229 / A25: real calls may proceed one at a time only after an exact current request quote and atomic USD30 reservation; a whole-task quote is no longer an entry prerequisite. Missing single-call cost, expired FX, insufficient remaining budget or unknown prior paid outcome pauses at that step and preserves checkpoints. The read-only preview reports current intent/embedding FX holds; no paid graph ran. [Evidence](PROGRESSIVE_BUDGET_ADMISSION_STAGE229_2026-09-14.md).
2026-09-14 stage 228: tenant-scoped 30-day workflow-stage usage now returns separate Tavily reported/estimated API credits, unknown attempts and observation-row coverage. Older rows stay unknown, and the compatibility mixed credit sum remains nonadditive; SQL fixture rolls back. [Evidence](TAVILY_CREDIT_USAGE_SQL_STAGE228_2026-09-14.md).
2026-09-14 stage 227: collect-evidence and correction stage metrics now retain Tavily reported-credit calls, estimated-credit calls and unknown retry attempts separately. The existing aggregate credit counter remains an operational estimate, not an invoice; no paid request or route changes. [Evidence](TAVILY_CREDIT_PROVENANCE_STAGE227_2026-09-14.md).
2026-09-14 stage 226: the frozen Colombia report now directs readers to the current matrix for O01–O05 implementation status; the matrix distinguishes latest synthetic Agent/SQL/browser evidence from the unrun real A11 graph. No workflow execution or route changed. [Evidence](CURRENT_STATUS_RECONCILIATION_STAGE226_2026-09-14.md).
2026-09-14 stage 225: official Gemini 2.5 Flash documentation makes grounding per prompt and supports the current Interactions parameter types, but does not provide a proven total tool-use-token ceiling for this agentic Google Search request. No model/route/tariff changed, and A11's whole-run paid gate remains closed. [Evidence](A11_GEMINI25_INTERACTIONS_BOUND_AUDIT_2026-09-14.md).
2026-09-14 stage 224: a 151-source/151-finding synthetic current-Agent graph now saves an OpenAI-compatible fallback phase in product SQL, survives controlled interruption, finishes missing phases plus fallback final in another process, and replays with no call in a third. Actual wire/fingerprints and cleanup pass; real model quality, default fallback tariff and A11 still require acceptance. [Evidence](P06_151_SOURCE_FALLBACK_CROSS_PROCESS_SQL_2026-09-14.md).
2026-09-14 stage 223: the current Agent's 151-source/155-finding synthetic phase/final graph now verifies every approved primary/actual OpenAI-compatible fallback wire, completes via a fallback phase and final score, and replays from in-memory checkpoints without new calls. Combined large-graph SQL recovery, default fallback tariff and real model semantics remain open. [Evidence](P06_151_SOURCE_FALLBACK_GRAPH_WIRE_2026-09-14.md).
2026-09-14 stage 222: a synthetic 151-source company with a very long critical role finding stays `retry-required` when the complete chunk/phase/final route cannot be bounded. The Agent sends no chunk and preserves the original facts; this is a no-paid safety boundary, not a completed large-company score. [Evidence](P06_WIDE_CRITICAL_PREFLIGHT_BOUNDARY_2026-09-14.md).
2026-09-14 stage 221: the due official FX refresh again returned only the expired Sep 11 date, retaining the Kimi/Beijing-embedding paid hold and 12:50:36 UTC next retry. No graph job or paid provider call ran; A11 still has no whole-run quote. [Evidence](FX_SCHEDULED_RECHECK_1150_2026-09-14.md).
2026-09-14 stage 220: the current local production build's task, recovery, country, cost and usage views passed 68 isolated desktop/mobile Chrome/HTTP/SQL checks. The read-only minimal-run preview still has no whole-run bound, so no graph job was claimed or paid provider call sent. [Evidence](CURRENT_BUILD_UI_AND_MATRIX_RECONCILIATION_2026-09-14.md).
2026-09-14 stage 219: qualification singleton planning now sizes each segment against every approved actual primary/fallback request contract, not the primary-only serialization. A synthetic OpenAI-compatible fallback completion reloaded from the real SQL checkpoint in a second process under its own identity; no real provider or default fallback tariff was admitted. [Evidence](P06_SINGLETON_FALLBACK_WIRE_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 218: the current Agent with product PostgreSQL checkpoints was run across two local processes for a long critical finding: the second process loaded the first segment and completed only missing synthetic model work, and a third call made no model request. Fixture cleanup passed. The full production graph and real model/customer chain remain unverified. [Evidence](P06_SINGLETON_AGENT_CROSS_PROCESS_SQL_2026-09-14.md).
2026-09-14 stage 217: after the scoring graph selects an oversized current company, the qualification Agent now proves a bounded full chunk/phase/final route before any chunk, loads or saves each completed original-unit segment under exact paid identity, and carries complete derived summaries to final scoring. Interrupted synthetic work resumed missing segments only; uncertain critical text remains pending. The real model, full large-source graph and A11 closed loop remain unverified. [Evidence](P06_SINGLETON_AGENT_RUNTIME_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 216: the qualification fact planner now exposes which original current source or corrected finding exceeded the phase byte limit, including citation IDs. The scoring graph still leaves such a company technical-pending; chunk purchase/recovery and final-score wiring remain to implement. [Evidence](P06_OVERSIZED_UNIT_RECOVERY_IDENTITY_2026-09-14.md).
2026-09-14 stage 215: the due official FX refresh yielded no current reference; CNY-paid workflow admission remains held by the existing 72-hour rule. No graph node or paid provider call ran. [Evidence](FX_SCHEDULED_RECHECK_1050_2026-09-14.md).
2026-09-14 stage 202: the latest local production UI/API regression verified task progress, new/legacy stop labels, country results, recovery pause and cost/usage views in real Chrome desktop/mobile with synthetic checkpoint data. No live workflow node, paid provider request or customer job ran. [Evidence](PRODUCTION_UI_REGRESSION_STAGE202_2026-09-14.md).
...
2026-09-15 阶段 242：必要复核只有 `not-required`、`secondary-confirmed`、`judge-resolved` 才计入接受。`review-failed`、`targeted-research-required` 或缺失复核会把图停在 `recover_incomplete_processing`，不继续发现补数、handoff 或持久化；授权恢复保留已完成主评分，只重进必要复核。handoff 使用相同完成判定。进度投影新增 `pendingReview`，生产双视口验证失败检查点 `1/1/1`→完成 `0/0/0`及未知 owner 失败。与阶段 241 合并完成阶段 239 的受控产品层；真实 Terra/账单仍未通过。[证据](LAYERED_QUALITY_AND_FAILURE_PROGRESS_STAGE242_2026-09-15.md)。
...
2026-09-14 stage 206 A11: read-only official endpoint audits keep Terra review and Sol judge tariffs unadmitted despite unchanged public prices. Candidate single-call ceilings USD11.019202 and USD27.736500 do not establish a whole-run bound or permission to call; the current USD30 account has USD17.675596 remaining. [Evidence](A11_REVIEW_PUBLIC_BOUND_RECHECK_2026-09-14.md).
2026-09-14 stage 207 A05/A11: the 09:49 UTC scheduled no-key ECB refresh still returned a stale Sep 11 reference, so CNY-bound model stages remain held. The next stored retry is 10:49:51 UTC; no paid work or task was started. [Evidence](FX_SCHEDULED_RECHECK_0949_2026-09-14.md).
2026-09-14 stage 208 P06: exact-wire preflight can partition one oversized fact or evidence excerpt into complete, source-hashed chunks with original citation closure. Current graph execution does not yet consume these chunks; it keeps uncompressible singletons pending before paid transport. Checkpoint and bounded synthesis integration are the next required steps. [Evidence](P06_SINGLETON_CHUNK_PREFLIGHT_2026-09-14.md).
2026-09-14 stage 209 P06: singleton chunks may be saved in the existing append-only qualification phase table under a derived per-unit stream fingerprint and exact paid request identity. SQL validation covers two chunks, cross-process reuse and tenant/country isolation. The graph/agent does not yet execute or synthesize this new stream; incomplete or unknown paid work remains held. [Evidence](P06_SINGLETON_CHUNK_CHECKPOINT_2026-09-14.md).
2026-09-14 stage 210 P06: pure local assembly of supplied singleton chunk outputs now verifies ordered code-point coverage and original citations, retaining uncertain interpretation if any part is uncertain. It does not itself prove checkpoint persistence; the graph/qualification agent still does not consume the assembled result. Final request byte bounds and model semantic quality remain open. [Evidence](P06_SINGLETON_CHUNK_SYNTHESIS_PREFLIGHT_2026-09-14.md).
2026-09-14 stage 211: discovery adapters now bound accepted result items before registry/qualification, with a read-only target-one maximum of 240 route item slots rather than a unique-company estimate. No task or paid work ran; complete A11 tariff and cost preflight remains open. [Evidence](DISCOVERY_DOWNSTREAM_RESULT_CAP_2026-09-14.md).
2026-09-14 stage 212 / A22: the current Tavily basic Extract step is now covered by a strict 20-URL/USD0.032000 active contract and same-operation paid replay guard. Fixture SQL kept unknown cost occupied and blocked duplicate, over-budget and invalid URL-count work; no real provider ran. Conditional review contracts A23/A24 are user-approved for implementation but not yet active, and the A11 total-run bound remains unknown. [Evidence](TAVILY_EXTRACT_ACTIVE_CONTRACT_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 213 / A23-A24: conditional secondary review uses the approved Terra USD11.019202/8,192 output contract; disagreement judge uses the separate Sol USD27.736500/12,000 output contract. Task attribution and strict request fields were checked with isolated SQL reservations, duplicate blocking and zero real provider calls. Ordinary Sol and S01 retain their prior contracts; the whole-run A11 bound remains absent. [Evidence](OPENROUTER_REVIEW_ACTIVE_CONTRACT_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 214 / A08: discovery graph call telemetry now reports provider-returned rows/links separately from admitted items and normalized/unique company identities, with over-delivery and unusable rows as explicit discard reasons. Old cached call raw volume is not inferred. Full regression passed with zero paid calls; real qualified yield and A11 cost bound remain open. [Evidence](DISCOVERY_PROVIDER_VOLUME_TELEMETRY_2026-09-14.md).
2026-09-15 stage 244 / A30-A29: the independent-review candidate packet now creates a recursive external disclosure copy that preserves public URLs while removing email, phone, credential/private-key, ID and payment-card patterns. Two inspected Terra v2 requests resumed only the review node from the existing graph; both produced no valid output or trusted usage/cash report. Conservative occupancy is USD83.947114, no handoff/company was persisted, and further identical replay is stopped pending diagnostic evidence. [Evidence](A30_PUBLIC_EXTERNAL_DISCLOSURE_STAGE244_2026-09-15.md).
2026-09-15 stage 245 / A31-A30: `rag-grounded-answer` now selects `openrouter-sol-rag-answer-credits`, an OpenAI-only free-text Sol wire with 61,440 request bytes, 8,192 `max_tokens` and USD0.901120 conservative reservation. The graph filters retrieved chunks to explicit public-source types and redacts the external copy; internal training, mailbox and unmarked uploads cannot reach the model. Public endpoint, no-network SDK and SQL synthetic checks pass with zero inference. [Evidence](A31_RAG_ANSWER_CONTRACT_STAGE245_2026-09-15.md).
2026-09-15 stage 247 / A33: for the exact configured single owner, every product paid transport runs in observation mode: no tariff/budget/freeze/hold admission and no pre-call monetary reservation. The ledger writes an unbounded/unknown marker before network, then records provider-reported cost/usage, latency, retries and output utilization when available. Authentication, A30 disclosure controls, tenant isolation and concurrent/success replay protection remain. The same owner may make the one A32 Bedrock attempt after an OpenRouter-edge 401/403/429/5xx; no third route is added. [Evidence](A33_OWNER_PAID_OBSERVATION_MODE_STAGE247_2026-09-15.md).
2026-09-15 stage 248 / A34: `rag-grounded-answer` routes directly to Moonshot `kimi-k3`, requests one JSON object with the cited answer, uses `max_completion_tokens=4096`, and has no OpenRouter, Bedrock, SDK retry or second model. Retrieval and A30 external-disclosure filtering happen before this call; A33 records zero pre-reserve and actual/unknown post-call usage. One public synthetic live call passed with a valid KB citation. [Evidence](A34_KIMI_RAG_ANSWER_STAGE248_2026-09-15.md).
