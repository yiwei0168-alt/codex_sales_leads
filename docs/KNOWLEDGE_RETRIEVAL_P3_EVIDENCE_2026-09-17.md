...
初始验收覆盖 36 份输入；P4 为六类产品及公司/行业事实抽样扩展到 43 份输入（34 PDF、7 PPTX、1 XLSX、1 合成受控文本），最终为 454 单元成功、12 空白、3 待 OCR、0 失败。登记原件映射到 102 个文档关系，生成 10,871 个无 embedding 的 v2 影子块；generation 状态为 `validated` 但未激活。共享 XLSX 关联多个分类文档，因此文档关系数高于源文件数。
