...
2026-09-14 stage 239 / A28 resume scope: the controlled end-to-end fixture will distinguish raw provider responses, admitted candidates, distinct company identities, evidence-ready candidates and post-review final-qualified companies. In-scope transfers, duplicates, unresolved roles and missing evidence must not fill the target. Live stage238 search remains only a partial supplier chain; no new discovery or Colombia evaluation is performed by recording this plan. [Resume](RESUME_PRODUCT_ACCEPTANCE_STAGE239_2026-09-14.md).
2026-09-14 stage 238: the real S02 playbook drove a Spanish CO/Distributor target-one search through Brave and downstream Tavily evidence. DeepSeek discovery gate and role correction produced valid response records, but the graph stopped at unknown-cost Terra review before final-qualified count or search-run persistence. Provider calls and candidate/company counts are separate; this run proves neither market exhaustion nor target fill. [Evidence](S02_MINIMAL_LIVE_PARTIAL_STAGE238_2026-09-14.md).
2026-09-14 stage 237 / S02: the user-approved market-playbook request parameter v2 is now the upstream source for search queries. Its model, OpenAI-only routing, strict JSON, cost bound and selected-country/role scope are unchanged; the synthetic request contract passes, but real search yield and final qualified count remain unproven. A27 permits recording and narrowly bypassing technical acceptance blockers when necessary for the main product chain, without treating a supplier failure as market exhaustion. [Evidence](S02_PLAYBOOK_PARAMETER_ACCEPTANCE_STAGE237_2026-09-14.md).
2026-09-14 stage 214: provider-returned row/link volume and over-delivery are now recorded separately from bounded admitted items, normalized company identities and final qualified output. `rawResults` reflects new provider observations; cached historical rows without this field do not gain invented counts. Synthetic 5-to-3 provider and hybrid discards pass regression with no paid work. [Evidence](DISCOVERY_PROVIDER_VOLUME_TELEMETRY_2026-09-14.md).
2026-09-14 stage 212 / A22: the existing Tavily basic Extract evidence step now has an approved strict USD0.032000/20-URL contract through Sep 21, with canonical paid replay protection and synthetic SQL budget verification. It is evidence acquisition, not a new discovery route. No real provider call or full-run approval follows from this single-call contract. [Evidence](TAVILY_EXTRACT_ACTIVE_CONTRACT_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 211: every discovery adapter now caps downstream candidate `items` at the requested result count, including Brave, Exa and Places over-delivery. The read-only target-one route can accept at most 240 item slots across its four scheduled steps and five rounds, subject to earlier stops; slots are not distinct or qualified companies. Raw provider response volume and Gemini server-side Google search count remain unknown, so this is not a whole-run cost bound. [Evidence](DISCOVERY_DOWNSTREAM_RESULT_CAP_2026-09-14.md).
2026-09-14 stage 204: the read-only target-one A11 preview records at most 12 requested results per scheduled route step and 240 requested result slots across four steps and five rounds. Brave and Exa can pass over-delivered response items downstream, so actual returned-item volume remains unknown and the result-slot figure is not a unique-company or whole-run cost ceiling. Route, admission and history are unchanged. [Evidence](A11_REQUESTED_RESULT_SLOTS_PREFLIGHT_2026-09-14.md).
2026-09-14 stage 200: SearchAPI request and recovery dependency now resolve the same trimmed standard/alias key and base URL. Account changes beneath a blank standard key invalidate old paid-cache identity without automatic replay; this does not admit the currently unverified SearchAPI tariff or change the route. [Evidence](SEARCHAPI_ALIAS_RECOVERY_IDENTITY_2026-09-14.md).
2026-09-14 stage 198: the Gemini provider, discovery-session checkpoint dependency and read-only route preview share the trimmed actual model selection. A blank `GEMINI_DISCOVERY_MODEL` falls back to `GEMINI_SEARCH_MODEL`; changing that fallback invalidates the old checkpoint before paid recovery. The default remains `gemini-3.6-flash`; the server-side Google query count and whole-run cost remain unknown. [Evidence](GEMINI_DISCOVERY_MODEL_RECOVERY_IDENTITY_2026-09-14.md).
2026-09-14 stage 193 O03: two fresh rounds without an increase in post-review final-qualified companies are a bounded stagnation safety stop, not proof that the market was exhausted. New tasks emit `no-qualified-progress`; old `confirmed-exhaustion` remains readable and its same-scope continuation guard remains. No route, scope, query or provider was changed. [Evidence](O03_STAGNATION_STOP_REASON_ACCEPTANCE_2026-09-14.md).
2026-09-14 stage 192: the current production default discovery wrapper allows two provider HTTP attempts per executed step. Read-only A11 preflight shares that constant and bounds the current four-provider/five-round route to 10 HTTP attempts per provider, 40 overall. Gemini internal Google Search count stays unknown; this is not a full paid-cost bound. [Evidence](A11_DISCOVERY_HTTP_ATTEMPT_CEILING_2026-09-14.md).
2026-09-14 stage 191: the read-only minimal Distributor A11 preview exposes at most five scheduled route steps per current provider (Brave, Exa, SearchAPI, Gemini Full), 20 over the five-round safety limit. Provider retries, Gemini server-side searches and paid attempts remain unknown; this cannot be priced as a complete run. [Evidence](A11_DISCOVERY_ROUTE_ACTION_CEILING_2026-09-14.md).
2026-09-14 stage 185: a search continuation is an inserted lineage and excluded-domain snapshot. Migration 073 prevents its owner from updating or deleting the record after migration 014's broad grant replays. Proposal/idempotency, tenant isolation and conversation cascade deletion still pass in SQL fixtures. [Evidence](MEMORY_AUDIT_SEARCH_CONTINUATION_ACL_2026-09-14.md).
2026-09-14 stage 183: Gemini discovery records the official `grounding_tool_count` when valid, otherwise distinct nonempty queries from complete response steps; missing observation stays unknown rather than zero. This is downstream telemetry only, and the unbounded server-side search route remains held before paid transport. [Evidence](GEMINI_GROUNDING_USAGE_PROVENANCE_2026-09-14.md).
...
2026-09-13阶段34：搜索后的评分采用校正主角色的专属证据锚点，忽略原发现轨道；请求家族范围仍由O02路由守卫决定。O05不新增搜索、不扩范围、不统一调分。冻结374条校正记录契约覆盖为112可用/208主角色待定/54需恢复；这些不是新增合格数量。详见[规则覆盖产物](reports/COLOMBIA_ROLE_CONTRACT_COVERAGE_2026-09-13.json)。

2026-09-13 阶段33 / 策略1.7.0：四类各轨Brave核心，Places/Exa/Gemini按类别缺口与同任务已购贡献条件启用；连续两次成功新请求无新增才暂停可选工具，缓存/故障不计贡献失败。其他类别工具不变。跨轨相同请求复用不依赖内部标签，精确文本及实际请求上下文仍区分；新策略使旧会话依赖失效。无新增真实实验，不修改冻结测评。

2026-09-13 阶段32：发现轮内每次调用和门禁完成后保存检查点，恢复读取原run、调用收据和门禁，仅运行缺失工作。遥测与轮次汇总分别幂等，保存故障独立于供应商健康，费用未知仍沿底层指纹门禁保留。真实SQL及中断合成验证通过；跨轨实际请求去重与条件工具顺序仍待完成。

2026-09-13 阶段31：发现完成后将任务会话写入图检查点，下一轮复用工具健康、冷却、去重与已购结果；任务/请求/策略/供应商配置变化不能静默沿用旧会话。单轮内部付费调用后的即时保存与跨轨请求去重仍待完成。类别路由配置本阶段未改，无新搜索/测评。

2026-09-13 阶段30：真实路由SQL的幂等与用户/工作区/国家隔离通过。角色待判以role-unresolved部分结束，不能认定市场耗尽或增加无新增轮次；故障期间旧停滞计数也不证明耗尽。最终持久化后重新核对合格保存数，低于目标不沿用中间target-met；最终发现量使用累计唯一发现数。冻结测评及其数量不变。
...
2026-09-13 阶段26（D13/O01）：空角色 Hybrid 归为未完成，不能进入评分或公共完成缓存；合法多家族 Hybrid 保留。校正输出缺项、确定性兜底、评分未完成会停止补池并返回 processing-incomplete，不增加无新增合格轮次、不声明市场耗尽。完成结果和费用保留；恢复与跨类转移仍待后续实现，本次无新增搜索/测评。

2026-09-13后续确认：O01–O05已获用户明确采纳（D13），覆盖下文旧待确认文字；仍待逐项实现/验证，搜索策略本阶段未改动。阶段25仅新增OpenRouter完整费用报告核销，费用上界及真实搜索闭环仍待验收，见[当前矩阵](CURRENT_ACCEPTANCE_MATRIX_2026-09-13.md)。
