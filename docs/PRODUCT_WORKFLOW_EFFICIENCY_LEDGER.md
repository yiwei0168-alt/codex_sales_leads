...
2026-09-16 LG03 Studio connection ledger: configuration input was one additional exact browser origin, one tracing-default value and the existing three registered graphs. The configuration check loaded all three graphs and confirmed the Studio origin; one CORS preflight returned 204 with the exact `https://smith.langchain.com` allow-origin in 204.65 ms, and one local `/info` request returned 200 in 14.24 ms. All three valid outputs were used for connection acceptance, with zero retries and zero discarded outputs. Four adapter tests, TypeScript and targeted lint passed. No graph business output or real-user adoption is claimed. Model/search/SMTP inputs, tokens, API credits, cash cost, paid calls and provider retries were zero. Requests rejected by LangSmith login, organization permissions or browser policy remain external setup failures rather than valid graph outputs. Optimization opportunity: keep the allowlist exact, leave tracing opt-in, and avoid exposing the Agent Server beyond loopback for Studio convenience.
