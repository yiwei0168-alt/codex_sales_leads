...
v2.0.22 removes those derived fields from the key, retaining protocol, full packet, judge identity, actual model and complete judge output. A deterministic migration consumes 91 existing cache entries and writes new keys for 27 arbitrations while preserving legacy files for audit. It also normalizes the cached derived fields and records aggregate input/output/usage, zero tokens, zero API cost and latency as a formal migration event. A regression test proves that changing only derived role-family fields cannot alter the key. No external call, score, evidence or prior cost event changes. The resulting cache permits a fully offline final-report regeneration and prevents the same class of cost leak in later taxonomy-parser revisions.
