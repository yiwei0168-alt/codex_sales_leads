...
混合搜索机制的独立策略、决策和真实贡献测量规范维护在`docs/search-strategy/`。后续真实搜索必须把provider的原始发现、唯一/辅助贡献、轻量门禁、证据可用、最终角色、统一分数、用户展示/采用和实际成本串联到同一候选ID；相同引擎和机制默认不重复调用，双轨任务通过实时注册表避免重复补证和评分。
