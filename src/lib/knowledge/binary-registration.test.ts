...
it("refuses another account's upload before reading a local artifact",async()=>{
  m.query.mockResolvedValueOnce([{role:"admin"}]).mockResolvedValueOnce([]);
  expect(await registerExtractedSharedBinary("owner",request)).toMatchObject({status:"missing_input"});
  expect(m.query.mock.calls[1][2]).toEqual([request.jobId,"owner"]);
  expect(m.upsert).not.toHaveBeenCalled();
});
it("rejects an account without the administrator role",async()=>{
  m.query.mockResolvedValueOnce([{role:"member"}]);
  expect(await registerExtractedSharedBinary("owner",request)).toMatchObject({status:"missing_input"});
  expect(m.query).toHaveBeenCalledTimes(1);
});
it("rejects a changed source hash without indexing",async()=>{
  m.query.mockResolvedValueOnce([{role:"admin"}]).mockResolvedValueOnce([{id:request.jobId,source_sha256:"b".repeat(64),status:"extracted"}]);
  expect(await registerExtractedSharedBinary("owner",request)).toMatchObject({status:"missing_input"});
  expect(m.upsert).not.toHaveBeenCalled();
});
it("reuses a recorded registration without a second embedding",async()=>{
  m.query.mockResolvedValueOnce([{role:"admin"}]).mockResolvedValueOnce([{id:request.jobId,source_sha256:request.sourceSha256,status:"registered",published_document_id:"doc",published_asset_id:"asset"}]);
...
it("registers only hash-matched extracted content and reports RAG v3 as pending",async()=>{
  ...
  m.query.mockResolvedValueOnce([{role:"admin"}]).mockResolvedValueOnce([{id:request.jobId,collection_slug:"industry",status:"extracted",title:"Fixture",storage_key:"knowledge/fixture.pdf",extraction_artifact_key:"knowledge/fixture.json",source_sha256:sourceSha256,source_url:null,entity_key:null,metrics:{artifactSha256:digest(artifact)}}])
  ...
  expect(m.query.mock.calls[2][2]).toEqual([request.jobId,"owner","doc","asset",sourceSha256]);
});
