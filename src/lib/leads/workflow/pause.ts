...
export function checkpointInvocation(snapshot:{values?:Record<string,unknown>;next?:readonly string[]},userId:string,actionId:string,expectedPlan?:LeadSearchPlan){
  ...
  if(expectedPlan && (!values.plan || persistenceInputFingerprint(values.plan)!==persistenceInputFingerprint(expectedPlan))) {
    throw new Error("Checkpoint plan mismatch; preserve completed work and reconcile the original request before resuming");
  }
