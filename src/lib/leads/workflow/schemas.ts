...
const dimensionsSchema = z.object({
  channelRoleAndCustomerAccess: z.number().min(0).max(30),
  productAndUseCaseFit: z.number().min(0).max(25),
  targetMarketCoverage: z.number().min(0).max(20),
  partnershipExecutionCapability: z.number().min(0).max(15),
  strategicComplementarity: z.number().min(0).max(10),
});
...
export const leadAssessmentModelSchema = z.object({
  ...
  primaryRole: channelRoleSchema.nullable(),
