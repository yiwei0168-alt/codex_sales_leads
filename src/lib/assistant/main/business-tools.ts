...
import { registerVectorlessUpload } from "@/lib/knowledge/vectorless-registration";
...
export const businessTools = [
  ...
  defineTool({id:"knowledge_shared_binary_register",description:"After exact administrator approval, verify an owned locally extracted binary, register it without embeddings, and build its searchable document tree. The current v3 release remains unchanged.",role:"admin",effect:"publish",recovery:"idempotent",
    ...
    execute:async(i,c)=>{const registered=await registerVectorlessUpload(c.userId,i.jobId,{language:i.language,authorityLevel:i.authorityLevel,expectedSourceSha256:i.sourceSha256});return result(registered,{receipt:registered.assetId});}}),
  ...
  defineTool({ id:"web_search",description:"Research public questions with Gemini Google grounding and citations. Public queries may run without a separate confirmation; never include private documents, mail, credentials or policy text.",input:z.object({questions:z.array(z.string().min(2).max(2000)).min(1).max(5)}).strict(),cost:"unknown",effect:"read",connections:["gemini-search"],execute:async i=>{
    const answer=await searchExternalWithGemini(i.questions);return result(answer,{sources:answer.citations});
  } }),
  defineTool({ id:"search_channel",description:"Run one configured discovery channel from a supplied public query, independently of market planning and knowledge retrieval. Public queries may run without a separate confirmation; raw provider payloads are not returned.",input:z.object({provider:z.enum(DISCOVERY_PROVIDER_IDS),query:z.string().min(2).max(2000),countryCode:country,countryName:z.string().max(120),languageCode:z.string().max(20),maxResults:z.number().int().min(1).max(20),category:z.enum(SEARCH_CATEGORY_IDS),engine:z.enum(["google-grounded","google","bing","google-places","brave","exa"])}).strict(),cost:"unknown",effect:"read",connections:["discovery-provider"],execute:async i=>{
