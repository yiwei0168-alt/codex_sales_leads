...
export async function readTaskDetail(userId:string,id:string,kind:string){
  ...
  if(kind==="search"){
    ...
    const candidates=await tenantQuery(userId,`select a.company_name as "company",a.domain,a.primary_role as "role",a.eligible,a.total_score as "score",a.selected,a.reasons,a.risks,a.unknowns
      ...
      order by a.selected desc,a.total_score desc,a.id limit 101`,[userId,id]);
    return {kind,action,details:{candidates:candidates.slice(0,100),truncated:candidates.length>100}};
  }
  if(kind==="contacts"){
    ...
    const items=await tenantQuery(userId,`select c.canonical_name as company,i.status,i.phase,i.attempts,i.named_contact_count,i.email_count,i.search_credits_used,i.extract_credits_used,i.error_message
      ...
      join sales_company c on c.id=i.company_id where i.run_id=$2 and w.owner_id=$1 order by c.canonical_name limit 100`,[userId,id]);
    return {kind,details:{...rows[0],items}};
  }
  if(kind==="draft"){
