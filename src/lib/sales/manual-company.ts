...
export async function addManualCompany(userId:string,input:z.infer<typeof manualCompanySchema>) {
  ...
  return tenantTransaction(userId,async client=>{
    ...
    const duplicates=await client.query<{external_id:string}>(`select wc.candidate_id as external_id from sales_company c join user_company_market wc on wc.company_id=c.id
      where wc.workspace_id=$1 and wc.market_country_code=$2 and (($3<>'' and lower(c.domain)=lower($3)) or
      lower(coalesce(wc.record->>'displayName',c.canonical_name))=lower($4)) limit 1`,[workspace.id,country,domain,input.name]);
    ...
    const result=await client.query<{id:string;external_id:string}>(`insert into sales_company(external_id,canonical_name,domain,country_code,source_kind,record)
      ...
      [id,input.name,domain||`${id}.invalid`,country,JSON.stringify({id,displayName:input.name,domain})]);
    const company=result.rows[0];record.id=companyMarketCandidateId(company.id,country);
    await client.query(`insert into workspace_company(workspace_id,company_id,account_tier,supply_model,brand_involvement,opportunity_stage,priority,
      ...
      on conflict(workspace_id,company_id) do nothing`,[workspace.id,company.id,record.accountTier,country,JSON.stringify(record)]);
    const manualOverrides={displayName:input.name,legalName:input.name,userAdded:true,manuallyEdited:true,
      ...(input.role?{primaryBusinessRole:input.role,roles:[input.role],layer:record.layer}:{})};
    await client.query(`insert into workspace_company_market(workspace_id,company_id,country_code,candidate_id,record,user_overrides,provenance)
      values($1,$2,$3,$4,$5,$6,'user-added')`,[workspace.id,company.id,country,record.id,JSON.stringify(record),JSON.stringify(manualOverrides)]);
    await client.query(`insert into workspace_audit_event(workspace_id,actor_user_id,entity_type,entity_id,action,changes)
      values($1,$2,'company',$3,'company.manually-added',$4)`,[workspace.id,userId,record.id,JSON.stringify({country,source:"user",evaluated:false,
      efficiency:{inputItems:1,inputCharacters:input.name.length+input.website.length,validOutputItems:1,downstreamUsedItems:1,inputTokens:0,outputTokens:0,costUsd:0,paidSearchCredits:0,retries:0,latencyMs:Date.now()-startedAt,utilizationEfficiency:1,discardedReasonCounts:{},optimizationOpportunity:"Reuse candidate before paid discovery"}})]);
