...
export async function analyzeStoredRelationship(userId:string,country:string,from:string,to:string){
  ...
  function filtered(result:z.infer<typeof schema>){return result.suggestions.filter(item=>!blocked.has(item.type)&&relationshipCitationValid(item.quote,item.evidenceId,records)).map(item=>({...item,status:"pending",sourceUrl:evidence.find(source=>source.id===item.evidenceId)?.url??""}));}
  if(reserved.cached)return {suggestions:filtered(reserved.cached),cached:true,reason:"复用当前证据版本分析；已否定且无新证据的类型不会重新建议。"};
  try{const response=await createLeadAiProvider().execute({task:"relationship",modelVersion:model,promptVersion:"stored-relationship-v1",tenantScope:userId,dataClassification:"private-workspace",evidenceIds:evidence.map(item=>item.id),outputSchema:z.toJSONSchema(schema),
    ...
    await tenantQuery(userId,"update user_relationship_analysis set status='completed',result=$3,metrics=$4,updated_at=now() where user_id=$1 and id=$2",[userId,reserved.id,JSON.stringify(parsed),JSON.stringify(metrics)]);
    return {suggestions,cached:false,reason:"只分析已保存证据；建议尚未成为公司关系，请审核后保存。"};
  }catch(error){await tenantQuery(userId,"update user_relationship_analysis set status='failed',metrics=$3,updated_at=now() where user_id=$1 and id=$2",[userId,reserved.id,JSON.stringify({latencyMs:Date.now()-started,costUsd:null,status:"unknown-cost-no-auto-retry"})]);throw error;}
}
