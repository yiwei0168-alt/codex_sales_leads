...
export async function followUpContext(userId:string,parentId:string){
  const rows=await tenantQuery<{id:string;workspace_id:string|null;company_id:string|null;content_ciphertext:string;country_code:string|null;role:string|null}>(userId,
    `select m.id,m.workspace_id,m.company_id,m.content_ciphertext,m.market_country_code as country_code,wc.record->>'primaryBusinessRole' as role
     from outbound_mail m left join user_company_market wc on wc.company_id=m.company_id and wc.workspace_id=m.workspace_id and wc.market_country_code=m.market_country_code
  ...
  const [ancestors,memories,imported]=await Promise.all([
    tenantQuery<{id:string;content_ciphertext:string;sent_at:string}>(userId,`with recursive chain as (
      ...
      where p.user_id=$1 and p.company_id is not distinct from $3::uuid and p.market_country_code is not distinct from $4::text
        and p.workspace_id is not distinct from $5::uuid and c.depth<8 and p.status='sent'
    ...
    tenantQuery<{id:string;content_ciphertext:string;sent_at:string}>(userId,`select m.id,m.content_ciphertext,m.sent_at::text from mailbox_message m
      where m.user_id=$1 and m.direction='inbound' and m.content_ciphertext is not null
        and ($2::uuid is null or exists (select 1 from mailbox_message_company l where l.user_id=m.user_id and l.message_id=m.id
          and l.company_id=$2 and l.source in ('domain-match','user-confirmed')))
