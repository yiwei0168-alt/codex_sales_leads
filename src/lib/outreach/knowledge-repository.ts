...
export async function searchOutreachKnowledge(
    ...
    `with eligible as (
       select * from outreach_knowledge_item
        where approval_status='active' and embedding is not null
          and (visibility='shared' or owner_id=$6)
     ), vectors as (
  ...
  const selected: OutreachKnowledgeItem[] = [];
...
export async function insertFeedbackMemory(client: PoolClient, userId: string, input: {
  ...
  const result = await client.query<{ id: string }>(
    `insert into outreach_knowledge_item (
       owner_id, visibility, kind, external_id, title, content, market_codes, channel_roles,
       priority_weight, source_refs, embedding
     ) values ($1,'private','feedback-memory',$2,'Approved outreach feedback memory',$3,$4,$5,2.4,$6,$7::vector)
     returning id`,
    [userId, `feedback:${input.feedbackId}`, input.summary, input.marketCodes, input.channelRoles,
      JSON.stringify({ feedbackId: input.feedbackId, reason: input.reason, provenance: "agent-screened-user-feedback" }),
