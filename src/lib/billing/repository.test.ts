...
it("audits A33 while allowing owner-scoped unknown replay and financial admission",async()=>{
  ...
  expect(JSON.parse(query.mock.calls[2][1][7]).admissionOverride).toEqual({ruleId:"A33",allowBudgetOverage:true,
    allowUnknownReplay:true,allowFinancialAdmissionBypass:true});
});
it("records an exact-owner unbounded call as unknown cost without a fake positive reserve",async()=>{
  vi.stubEnv("PAID_CALL_STAGE_OVERRIDE","A33");
  vi.stubEnv("PAID_CALL_STAGE_OVERRIDE_USER_ID","owner");
  query.mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[]})
    .mockResolvedValueOnce({rows:[{id:"unbounded"}]}).mockResolvedValueOnce({rows:[]});
  await expect(reservePaidCall("owner",{...input,maximumChargeMicros:0,costBoundKnown:false,
    requestFingerprint:"a".repeat(64)})).resolves.toBe("unbounded");
  expect(query.mock.calls[2][1][5]).toBe(0);
  expect(query.mock.calls[2][1][6]).toBe(false);
  const metrics=JSON.parse(query.mock.calls[2][1][7]);
  expect(metrics).toMatchObject({costBoundKnown:false,usageBoundary:"a33-owner-authorized-unbounded-call-recorded-before-network"});
  expect(metrics.reservationAllocation.amountKnown).toBe(false);
});
...
it("preserves native reservation and the versioned FX source in the same reservation record",async()=>{
  ...
  expect(JSON.parse(query.mock.calls[2][1][7])).toMatchObject({foreignCostBound,fxReservationBufferPercent:5});
});
