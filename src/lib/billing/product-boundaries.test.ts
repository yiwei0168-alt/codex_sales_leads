...
it("blocks intent before transport without provider fallback or deterministic success",async()=>{
  ...
  await expect(withProductSpend("owner","intent",()=>planAssistantRequest("Find distributors",[],transport))).rejects.toMatchObject({name:"BudgetDeniedError",code:expect.stringMatching(/^(missing|expired)-tariff$/)});
