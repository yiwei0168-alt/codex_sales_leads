...
it("checks two independent official pages once and does not touch paid accounting",async()=>{
  const transport=vi.fn(async()=>new Response());
  const result=await refreshSearchRateEvidence(transport as typeof fetch,now);
  expect(result).toMatchObject({status:"checked",httpCalls:2,
  ...
  expect(mocks.evidence).toHaveBeenCalledTimes(2);expect(transport).toHaveBeenCalledTimes(2);
  ...
  expect(observations.map(item=>item.freeHttpRequests)).toEqual([1,1]);
  expect(observations.map(item=>item.validOutputItems)).toEqual([1,1]);
...
it("holds only the changed source and keeps its hold on later outages",async()=>{
  ...
  expect(result.states.map(item=>[item.status,item.hold])).toEqual([["unavailable",true],["unavailable",false]]);
...
it("returns explicit missing states for both Search entries",async()=>{
  mocks.read.mockResolvedValueOnce([]);
  expect((await readSearchRateStatuses()).map(item=>item.status)).toEqual(["missing","missing"]);
});
