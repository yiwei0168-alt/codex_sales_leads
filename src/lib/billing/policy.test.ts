...
it("expires rates after seven days or at the promotion boundary even if configuration expiry is later",()=>{
  expect(()=>quoteRequest(input,[rule],Date.parse("2026-09-19T00:00:00Z"))).toThrow("expired-tariff");
  expect(()=>quoteRequest(input,[{...rule,promotionEndsAt:"2026-09-13T00:00:00Z"}],Date.parse("2026-09-13T00:00:00Z"))).toThrow("expired-tariff");
  expect(()=>quoteRequest(input,[{...rule,verifiedAt:"not-a-date"}],Date.parse("2026-09-13T00:00:00Z"))).toThrow("expired-tariff");
});
