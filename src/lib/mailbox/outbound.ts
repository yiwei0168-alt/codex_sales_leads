...
export const sendMailSchema=z.object({connectionId:z.uuid(),companyExternalId:z.string().min(1).max(180),
  ...
  body:z.string().trim().min(1).max(30000),idempotencyKey:z.uuid(),parentId:z.uuid().optional(),confirmed:z.literal(true)}).strict();
...
export async function listOutbound(userId:string,companyExternalId:string,offset=0,limit=51) {
  const rows=await tenantQuery<{id:string;status:string;sent_at:string|null;created_at:string;content_ciphertext:string;parent_id:string|null;error_code:string|null}>(userId,
    `select m.id,m.status,m.sent_at::text,m.created_at::text,m.content_ciphertext,m.parent_id,m.error_code from outbound_mail m
     join sales_company c on c.id=m.company_id where m.user_id=$1 and c.external_id=$2 order by m.created_at desc,m.id desc limit $3 offset $4`,[userId,companyExternalId,Math.min(101,Math.max(1,limit)),Math.max(0,offset)]);
  return rows.map(row=>({id:row.id,status:row.status,sentAt:row.sent_at,createdAt:row.created_at,parentId:row.parent_id,reconciledByUser:row.error_code?.startsWith("USER_CONFIRMED_")??false,
...
export async function reconcileOutbound(userId:string,input:z.infer<typeof reconcileMailSchema>){
  return tenantTransaction(userId,async client=>{
    const rows=await client.query<{id:string;workspace_id:string;company_id:string;status:string;eligible:boolean}>(`select id,workspace_id,company_id,status,
    ...
    if(input.outcome==="sent")await client.query(`update workspace_company set opportunity_stage='Contacted',manually_edited=true,updated_at=now()
      where workspace_id=$1 and company_id=$2 and opportunity_stage in ('Discovered','Qualified','Priority','Contact Prepared')`,[row.workspace_id,row.company_id]);
...
export async function sendOutbound(userId:string,input:SendInput) {
  ...
  const reserved=await tenantTransaction(userId,async client=>{
    const company=await client.query<{id:string;workspace_id:string}>(`select c.id,w.id as workspace_id from sales_company c
      join workspace_company wc on wc.company_id=c.id join market_workspace w on w.id=wc.workspace_id
      where c.external_id=$1 and w.owner_id=$2 and w.slug='global-sales'`,[input.companyExternalId,userId]);
    ...
    if(input.parentId){const parent=await client.query<{message_id:string;content_ciphertext:string}>(`select message_id,content_ciphertext from outbound_mail
      where id=$1 and user_id=$2 and company_id=$3 and connection_id=$4 and status='sent'`,[input.parentId,userId,company.rows[0].id,input.connectionId]);
    ...
    const saved=await client.query<{id:string}>(`insert into outbound_mail(user_id,workspace_id,company_id,connection_id,idempotency_key,
      request_hash,message_id,parent_id,content_ciphertext,status) values($1,$2,$3,$4,$5,$6,$7,$8,$9,'sending')
      on conflict do nothing returning id`,[userId,company.rows[0].workspace_id,company.rows[0].id,input.connectionId,input.idempotencyKey,hash,messageId,input.parentId??null,encrypted]);
  ...
  try {
    ...
    await tenantTransaction(userId,async client=>{
      ...
      await client.query(`update workspace_company wc set opportunity_stage='Contacted',manually_edited=true,updated_at=now() from outbound_mail m
        where m.id=$1 and m.user_id=$2 and wc.workspace_id=m.workspace_id and wc.company_id=m.company_id
        and wc.opportunity_stage in ('Discovered','Qualified','Priority','Contact Prepared')`,[reserved.id,userId]);
    });
  }catch(error){
