...
测评和产品中的证据补全必须先把候选归并到规范公司实体，再为每家公司建立一份工具中立的共享证据档案，供该公司在不同搜索工具和不同提交通道中的全部出现记录共同使用。规范身份优先使用官网域名；只有在官网域名不冲突时，才使用强公司名、品牌名或官方平台别名辅助合并。第三方目录的域名不能作为公司别名。直接抓取的页面除了域名一致，还必须在正文中匹配公司／品牌；输入记录包含法律实体名称时，也必须匹配相应法律实体别名，不能因为同域就默认主体正确。

所有搜索摘要和 provider 原生公司总结只能保留用于规划抓取目标与实验追溯，必须从评分证据视图中排除。每家公司最多直接抓取五个官网页面，再使用最多两个回退来源；顺序为官网直连、对失败官网 URL 使用 Tavily Extract、再由 Tavily／Exa 寻找带可审计 URL 的原始页面内容。搜索摘要本身仍不能转为事实证据。所需身份、市场、主动网络业务、提交通道和合作路径信号已经得到支持时可以提前停止；小型长尾单来源例外继续适用。抓取失败或预算耗尽只保留 unknown，不能转换成负面公司事实。
