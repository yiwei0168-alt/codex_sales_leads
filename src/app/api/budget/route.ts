...
export async function GET(){
  ...
  try{return Response.json({...await readSpendBudget(session.userId),tariffVersion:billingPolicy.version,configuredRules:billingPolicy.rules.length,
    notice:"当前接入后台线索工作流；独立聊天、邮件及联系人入口尚未全量接入。累计预算不自动按月重置。预留占用不等于实际账单；未知费用保留占用，缺价或费用上界不明时阻止已接入的付费请求。"},{headers:{"Cache-Control":"private, no-store"}});}catch{return Response.json({error:"预算读取失败；不能推断可用金额"},{status:503});}
}
