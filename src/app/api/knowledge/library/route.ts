...
export async function GET(request:Request){
  ...
  const parsed=z.object({scope:z.enum(["all","private","shared","evidence"]),query:z.string().max(200),offset:z.coerce.number().int().min(0).max(100000)}).safeParse({scope:params.get("scope")??"private",query:params.get("q")??"",offset:params.get("offset")??0});
  if(!parsed.success)return Response.json({error:"筛选参数无效"},{status:400});const started=Date.now();const p=parsed.data;
  try{const page=await listKnowledgeLibrary(session.userId,p.scope,p.query,p.offset,12);
