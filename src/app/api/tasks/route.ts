...
const schema=z.object({kind:z.enum(["all","search","contacts","draft","send"]),country:z.string().regex(/^(all|unknown|[A-Z]{2})$/),status:z.enum(["all","active","attention","finished"]),offset:z.coerce.number().int().min(0).max(100000)});
