...
export async function GET(){
  ...
  try{
    const [rows,billing,workflowStages,workflowModels]=await Promise.all([
      tenantQuery(session.userId,`select stage,count(*)::int as operations,
      ...
      tenantQuery(session.userId,WORKFLOW_STAGE_USAGE_SQL,[session.userId]),
      tenantQuery(session.userId,WORKFLOW_MODEL_USAGE_SQL,[session.userId]),
    ]);
    return Response.json({scope:"operations-and-http-attempts-separate",periodDays:30,stages:rows,billingStages:billing,
      workflowStageMetrics:workflowStages,workflowModelUsage:workflowModels,userAdoptionCoverage:"unknown",
      totalCostComplete:false,
      notice:"操作、HTTP预留、工作流阶段与模型用量是不同且有重叠的记录，金额和数量不得跨表相加。工作流阶段的下游使用不是用户采用；未知采用、费用和历史未接入调用不填零。预留不是实际花费；延迟是记录耗时之和，不是并行任务墙钟时间。"},{headers:{"Cache-Control":"private, no-store"}});
