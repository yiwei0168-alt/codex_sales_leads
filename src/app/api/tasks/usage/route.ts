...
export async function GET(){
  ...
  try{
    ...
    return Response.json({scope:"new-non-lead-operations-only",periodDays:30,stages:rows,totalCostComplete:false,notice:"仅统计新接入的意图识别及本地查询记录；不包含线索搜索、补证或邮件生成账单。未知费用未按零计算。"},{headers:{"Cache-Control":"private, no-store"}});
