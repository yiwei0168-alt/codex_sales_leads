...
const needsReview=(skill:Skill)=>skill.validation.scripts!=="none"||skill.validation.dependencies!=="none";
...
export function MemorySkills(){
  ...
  return <section className="memory-skills" aria-label="Skill">
    ...
    <div className="memory-skill-list">{items.map(skill=><article key={skill.id} className="memory-skill-row">
      <div><strong>{skill.name}</strong><small>v{skill.current_version} · {skill.scope==="global"?"全局":"本账户"} · {skill.enabled?(skill.scope==="global"&&!skill.published?"待发布":"已启用"):needsReview(skill)?"待审核":skill.validation.autoEnable==="pending-replay-and-shadow"?"待验证":"已停用"}</small>
        <small>来源：{skill.source}</small></div>
      {skill.owned&&<div className="memory-skill-actions">
        <button type="button" disabled={busy||(!skill.enabled&&needsReview(skill))}
