...
type Item={id:string;title:string;sourceUrl?:string;assetId?:string;documentType?:string;version?:string;updatedAt:string;status:string;scope:string;excerpt?:string;
  treeStatus?:"searchable"|"pending"|"unavailable";treeVersionId?:string|null;sourceSha256?:string|null};
const PAGE_SIZE=12;
export function KnowledgeLibrary(){
  ...
  useEffect(()=>{
    ...
    fetch(`/api/knowledge/library/${selected.id}?scope=${selected.scope==="public-evidence"?"evidence":selected.scope}`,{signal:controller.signal,cache:"no-store"})
  ...
  function sources(item:Item){return <div className="library-links">{item.assetId&&<a href={`/api/knowledge/assets/${item.assetId}`} target="_blank" rel="noreferrer">打开原始资料</a>}{item.sourceUrl&&/^https?:\/\//i.test(item.sourceUrl)&&<a href={item.sourceUrl} target="_blank" rel="noreferrer">查看来源</a>}{item.scope==="private"&&<KnowledgeRevisions documentId={item.id}/>}</div>;}
  return <section className="knowledge-library" aria-label="资料列表">
    {selected ? <article className="library-reader"><button className="secondary-button" onClick={()=>{setSelected(null);setContent(null);setError("");}}>返回资料列表</button><h2>{selected.title}</h2>{sources(selected)}{selected.treeStatus&&<p className="subtle">{selected.treeStatus==="searchable"?"可检索":selected.treeStatus==="pending"?"待建立索引":"来源或版本已失效"}{selected.treeVersionId?` · 树版本 ${selected.treeVersionId.slice(0,8)}`:""}{selected.sourceSha256?` · 来源 ${selected.sourceSha256.slice(0,8)}`:""}</p>}{reading&&<p role="status">正在读取资料…</p>}{error&&<p role="alert">{error}<button onClick={()=>{setReading(true);setRevision(value=>value+1);}}>重试</button></p>}{selected.scope!=="public-evidence"&&<KnowledgeTree key={selected.id} documentId={selected.id}/>}<details className="knowledge-legacy-text"><summary>查看已保存的检索正文</summary><p className="subtle">检索片段可能含重叠内容；原始排版请查看原始资料。</p><div className="library-body">{content&&<SavedKnowledgeText text={content}/>}</div></details></article> : <>
    <div className="library-toolbar"><label>范围<select aria-label="知识范围" value={scope} onChange={event=>{setScope(event.target.value);setOffset(0);setItems([]);}}><option value="all">全部</option><option value="private">我的私有知识</option><option value="shared">共享知识（只读）</option><option value="evidence">公共证据库（只读）</option></select></label>
    ...
    <div className="library-list">{items.map(item=><article className="library-row" key={item.id}><div><button className="library-title" onClick={()=>{setSelected(item);setContent(null);setError("");setReading(true);}}>{item.title}</button><p>{scope==="all"?`${item.scope==="private"?"我的私有知识":item.scope==="shared"?"共享知识":"公共证据库"} · `:""}{item.status} · {item.documentType??"已保存文本"}{item.version?` · ${item.version}`:""} · {item.updatedAt?.slice(0,10)}</p>{item.treeStatus&&<p>{item.treeStatus==="searchable"?"可检索":item.treeStatus==="pending"?"待建立索引":"来源或版本已失效"}{item.treeVersionId?` · 版本 ${item.treeVersionId.slice(0,8)}`:""}{item.sourceSha256?` · 来源 ${item.sourceSha256.slice(0,8)}`:""}</p>}<p className="library-excerpt">{item.excerpt??"无文本摘要"}</p>{sources(item)}</div>{item.scope==="private"&&<button className="text-button" onClick={()=>void remove(item)}>删除此私有知识</button>}</article>)}</div>
    {!error&&!items.length&&<p className="empty-state">此范围暂无资料。可以调整筛选，或通过“上传资料”加入知识。</p>}
    <div className="library-pagination"><button disabled={offset===0} onClick={()=>setOffset(value=>Math.max(0,value-PAGE_SIZE))}>上一页</button><span>第 {offset/PAGE_SIZE+1} 页 · 每页 {PAGE_SIZE} 条</span><button disabled={!more} onClick={()=>setOffset(value=>value+PAGE_SIZE)}>下一页</button></div>
    </>}
  </section>;
}
