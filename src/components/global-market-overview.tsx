...
export function GlobalMarketOverview({companies,country="all"}:{companies:CompanyRecord[];country?:string}){
  ...
  return <section className="panel"><h2>已保存市场概览</h2><p>仅统计当前用户数据库记录；不推断市场覆盖率或未搜索地区的机会。</p>
    ...
    <div className="table-scroll"><table className="data-table responsive-table"><thead><tr><th>国家</th><th>候选</th><th>开发名单</th><th>已联系及后续</th><th>到期待跟进</th><th>排队 / 运行任务</th></tr></thead><tbody>{codes.map(code=>{
      const rows=companies.filter(company=>marketCode(company.country)===code);
      return <tr key={code}><td data-label="国家"><a href={marketHref(code,"leads")}>{marketLabel(code)}</a></td><td data-label="候选">{rows.length}</td>
        <td data-label="开发名单">{rows.filter(item=>!["Discovered","Excluded"].includes(item.opportunityStage)).length}</td>
        <td data-label="已联系及后续">{rows.filter(item=>["Contacted","Engaged","Cooperating"].includes(item.opportunityStage)||Boolean(item.outreachSummary?.sentCount)).length}</td>
        <td data-label="到期待跟进">{rows.filter(item=>item.nextActionDueAt&&item.nextActionDueAt<=today&&!["Closed","Paused","Discovered","Excluded"].includes(item.opportunityStage)).length}</td><td data-label="排队 / 运行任务">{error?"不可用":tasks===null?"读取中":tasks.find(item=>item.country===code)?.active??0}</td></tr>;
