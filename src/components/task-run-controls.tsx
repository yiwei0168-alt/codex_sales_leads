...
export function TaskRunControls({actionId,status}:{actionId:string;status:string}){
  ...
  useEffect(()=>{const controller=new AbortController();let pending=false;
    async function read(){if(pending||document.hidden)return;pending=true;try{const response=await fetch(`/api/assistant/actions/${actionId}/progress`,{signal:controller.signal,cache:"no-store"});if(!response.ok)throw new Error();const data=await response.json();if(!controller.signal.aborted)setState(data);}catch{if(!controller.signal.aborted)setMessage("进度读取失败，可刷新重试");}finally{pending=false;}}
    void read();const timer=window.setInterval(()=>void read(),10000);return()=>{controller.abort();window.clearInterval(timer);};
  },[actionId]);
  async function action(pause:boolean){if(busy)return;setBusy(true);setMessage("");try{const response=await fetch(`/api/assistant/actions/${actionId}/${pause?"progress":"confirm"}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:pause?"pause":"resume"})});const body=await response.json();if(!response.ok)throw new Error(body.error);setMessage(pause?"已请求暂停：当前阶段会完成并保存，随后不再启动下一阶段。":"任务已恢复，可在任务中心跟踪。");router.refresh();}catch(error){setMessage(String(error));}finally{setBusy(false);}}
