  ...
  return <div className="knowledge-layout">
    ...
    <div className="kb-main-grid">
      <section className="panel rag-playground">
        ...
        {answer && <div className="rag-result"><div className="rag-result-meta"><span className={answer.grounded ? "grounded" : "ungrounded"}>{answer.kind ?? (answer.grounded ? "Grounded" : "Needs review")}</span><span>{answer.reasonCode ?? answer.model}</span><span>{answer.latencyMs} ms</span></div><div className="rag-answer">{answer.answer}</div>{answer.warnings.map((warning) => <p className="rag-warning" key={warning}>⚠ {warning}</p>)}{(answer.documents?.length ?? 0) > 0 && <div className="rag-citations"><strong>原始资料 · {answer.documents?.length}</strong>{answer.documents?.map((document) => <a key={document.assetId} href={document.url} target="_blank" rel="noreferrer"><span>{document.documentType}</span><div><b>{document.title}</b><small>{document.version ?? "未标注版本"}</small></div></a>)}</div>}{(answer.factCitations?.length ?? 0) > 0 && <div className="rag-citations"><strong>已验证事实 · {answer.factCitations?.length}</strong>{answer.factCitations?.map((citation) => <a key={citation.factId} href={`/api/knowledge/assets/${citation.assetId}`} target="_blank" rel="noreferrer"><span>{citation.attributeKey}</span><div><b>{citation.rawValue}</b><small>{citation.version ?? "未标注版本"}</small></div></a>)}</div>}<div className="rag-citations"><strong>检索证据 · {answer.citations.length}</strong>{answer.citations.map((citation) => <a key={citation.chunkId} href={citation.sourceUrl} target="_blank" rel="noreferrer"><span>[KB:{citation.chunkId.slice(0, 8)}…]</span><div><b>{citation.documentTitle} · {citation.visibility === "private" ? "私有" : "共享"}</b><small>{citation.excerpt}</small></div><em>{Math.round(citation.score * 100)}%</em></a>)}</div></div>}
      </section>
