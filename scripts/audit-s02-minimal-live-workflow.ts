...
try {
  ...
  const [reservations, stages, runs, models, assessments, checkpoints, reviewCheckpoints] = await Promise.all([
    tenantQuery(userId,`select stage,tariff_key,status,reserved_micros::text,reported_micros::text,settled_micros::text,
  ...
  console.log(JSON.stringify({actionId,job,reservations,stages,runs,models,assessments,checkpoints,reviewCheckpoints},null,2));
} finally {
  await getPool().end();
}
