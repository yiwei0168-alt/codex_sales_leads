...
try {
  ...
  console.log(JSON.stringify({ runId: run.id, mode: "shadow", processed, routineModel, escalationModel, timeoutMs }, null, 2));
} catch (error) {
  await query(
    `update contact_verification_run set status = 'failed', error_message = $2, finished_at = now() where id = $1`,
    [run.id, error instanceof Error ? error.message : "Unknown contact verification error"],
  );
  throw error;
} finally {
  await getPool().end();
