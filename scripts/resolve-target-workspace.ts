  ...
  const rows = requestedUserEmail
    ? await query<{ id: string; email: string }>(
      `select w.id, u.email from market_workspace w join app_user u on u.id = w.owner_id
       where w.slug = 'mexico-pilot' and w.status = 'active' and u.status = 'active' and lower(u.email) = $1`,
      [requestedUserEmail],
    )
    : await query<{ id: string; email: string }>(
      `select w.id, u.email from market_workspace w join app_user u on u.id = w.owner_id
       where w.slug = 'mexico-pilot' and w.status = 'active' and u.status = 'active'
       order by u.created_at`,
    );
  if (rows.length === 0) throw new Error(requestedUserEmail
    ? `No active mexico-pilot workspace found for ${requestedUserEmail}`
    : "No active mexico-pilot workspace found");
