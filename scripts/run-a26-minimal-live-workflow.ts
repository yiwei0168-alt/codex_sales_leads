...
const title="Find one networking distributor in Colombia for local product acceptance. Use Spanish search queries and keep the target at one company.".slice(0,38);
...
try{
  ...
  const previousPaid=await tenantQuery<{n:number}>(userId,
    ...
    [userId,action.id]);
  const safeWorkspaceRecovery=action.status==="failed"&&action.errorMessage==="Global sales workspace not found"
    &&previousPaid[0]?.n===0;
  if(!process.argv.includes("--run")||action.status!=="proposed"&&!safeWorkspaceRecovery){
    ...
    const claim=await claimLeadWorkflowByAction(userId,action.id,"a26-local-isolated-once");
