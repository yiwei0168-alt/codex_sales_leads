...
let created=false,discoveryCalls=0,providerCalls=0,activeThreadId=threadId;
try{
  const client=await admin.connect();
  try{
    await client.query("begin");
    for(const [id,address] of [[userId,email],[otherUserId,otherEmail]])
      await client.query("insert into app_user(id,email,display_name,role,status) values($1,$2,'Zero graph fixture','member','disabled')",[id,address]);
  ...
  console.log(JSON.stringify({graphSqlCompletion:"passed",target:1,accepted:0,stopReason:"provider-unavailable",discoveryCalls,
    providerCalls,completionReceipts:receipts.length,isolatedSearchTables:5,
  ...
  if(created){
    ...
    try{
      ...
      await saver.deleteThread(activeThreadId).catch(()=>undefined);
