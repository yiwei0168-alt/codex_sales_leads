...
const prompt="Find one networking distributor in Colombia for local product acceptance. Use Spanish search queries and keep the target at one company.";
const correction="Keep the Colombia Distributor target at exactly one. Set search queryLanguage to es (Spanish), not en. Please update the proposed plan.";
...
try{
  ...
  if(!process.argv.includes("--run")){
    ...
    const conversation=needsCorrection
      ?await withSpendContext({userId,operationId:"a26-local-minimal-language-correction",stage:"validation",
          fixedFxReferenceVersion:fxVersion},()=>processAssistantMessage(userId,
          {conversationId:prior!.id,content:correction}))
      :prior??await withSpendContext({userId,operationId:"a26-local-minimal-intent",stage:"validation",
