...
if(mode==="probe"){
  ...
  const loaded=await productQualificationPhaseCheckpoint({userId,workspaceId,actionId,
    countryCode:"CO",expectedProviderId:"deepseek"}).load(request,contract!);
  ...
  try{
    ...
    await own.save(request,contract!,response);
    assert.deepEqual(await own.load(request,contract!),response);
    ...
    await assert.rejects(own.save(request,contract!,{...response,output:{...response.output as object,
      facts:[{findingId:"fixture-f1",materiality:"material",summary:"Changed.",evidenceIds:["fixture-e1"]}]}}),
    /conflict/);
    await assert.rejects(own.save(request,contract!,{...response,output:{facts:[],sources:[]}}),
      /missing or extra/);
    await assert.rejects(own.save(request,contract!,{...response,actualProviderId:"fallback-provider"}),
    ...
    assert.equal(await productQualificationPhaseCheckpoint({userId,workspaceId,actionId,
      ...
      market:{countryCode:"MX"}}},contract!),null);
    assert.equal(await productQualificationPhaseCheckpoint({userId:otherUserId,workspaceId:otherWorkspaceId,
      actionId:otherActionId,countryCode:"CO",expectedProviderId:"deepseek"}).load(request,contract!),null);
    ...
    await assert.rejects(productQualificationPhaseCheckpoint({userId:otherUserId,workspaceId,
      actionId,countryCode:"CO",expectedProviderId:"deepseek"}).save(request,contract!,response),
    ...
    console.log(JSON.stringify({phaseCheckpointSql:"passed",crossProcessLoads:1,completedRows:1,
      duplicateRows:0,foreignReads:0,crossCountryReads:0,mutationDenied:true,paidProviderCalls:0}));
  }finally{
    if(created){
      const client=await admin.connect();
      try{
