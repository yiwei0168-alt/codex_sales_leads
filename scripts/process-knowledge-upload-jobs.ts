...
try{
  ...
  for(let index=0;index<limit;index++){
    let job:{id:string;storageKey:string;visibility:string;userId:string}|null=null;
    for(const account of accounts){
      job=await tenantTransaction(account.id,async client=>{
        const selected=await client.query<{id:string;storageKey:string;visibility:string}>(`select id,storage_key as "storageKey",visibility from knowledge_upload_job where user_id=$1 and status='pending' order by created_at for update skip locked limit 1`,[account.id]);
