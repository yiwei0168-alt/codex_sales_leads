...
## Frozen business request audit

After rebuilding, run `node scripts/run-tsx.cjs scripts/offline-tokenizer/measure-frozen.ts` from the project root. It requires the two existing local German 207-company snapshots; do not commit them. Current private request builders are accessed diagnostically (their signatures are checked by successful execution, not treated as a public API). Qualification uses a standard playbook without historical RAG/private memory; no scoring runs.

The provider transport is mocked; raw request bodies are held in memory and passed over stdin into a disposable `--network none` container, without files, mounts, or injected credentials. Only aggregate statistics are printed. All 207 single-company requests and existing input-character-based batches are reconstructed for each of three stages. This is not a simulation of the proposed byte-based batching algorithm. Eight components are tokenized in isolation; they overlap/nest and cannot be summed into total tokens. Null/empty placeholders also encode to tokens. Full prompt totals use protocol conversion plus V4 encoding, not tokenization of the HTTP envelope.
