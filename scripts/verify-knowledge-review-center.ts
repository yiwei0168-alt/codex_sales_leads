...
try{await pool.query("insert into app_user(id,email,display_name,password_hash,role,status) values($1,$2,'Review UI fixture',$3,'admin','active')",[userId,email,hashPassword(password)]);await pool.query("insert into market_workspace(id,owner_id,slug,name,market,country_code,objective) values($1,$2,'global-sales','Review UI fixture','Global','WW','Review center verification')",[workspaceId,userId]);created=true;
  const checks:string[]=[];let factReviews=0,goldReviewed=0,goldTotal=0,holdoutUnlocked=false;for(const viewport of [{width:1366,height:900},{width:390,height:844}]){const context=await browser.newContext({viewport});await context.route("**/*",route=>{const url=new URL(route.request().url());return["localhost","127.0.0.1"].includes(url.hostname)?route.continue():route.abort();});const page=await context.newPage();await page.goto(base.href);await page.getByLabel("登录邮箱").fill(email);await page.getByLabel("密码",{exact:true}).fill(password);await page.getByRole("button",{name:"登录",exact:true}).click();
    if(viewport.width<600)await page.getByRole("button",{name:"打开导航菜单"}).click();await page.getByRole("button",{name:"知识库 & RAG"}).click();await expect(page.getByRole("button",{name:"进入复核中心"})).toBeVisible();await page.getByRole("button",{name:"进入复核中心"}).click();await expect(page.getByRole("tab",{name:"事实复核"})).toBeVisible();
    const factApi=await page.evaluate(async()=>{const response=await fetch("/api/knowledge/reviews?limit=1");return{status:response.status,body:await response.json()};});expect(factApi.status).toBe(200);expect(factApi.body.limit).toBe(1);factReviews=factApi.body.total;await expect(page.getByText(String(factReviews),{exact:true}).first()).toBeVisible();
    await page.getByRole("tab",{name:"Gold 审核"}).click();const goldSummary=await page.evaluate(async()=>{const response=await fetch("/api/knowledge/evaluation-reviews?limit=1");return{status:response.status,body:await response.json()};});expect(goldSummary.status).toBe(200);goldReviewed=goldSummary.body.reviewed;goldTotal=goldSummary.body.total;holdoutUnlocked=goldSummary.body.holdoutUnlocked;await expect(page.getByText(`${goldReviewed}/${goldTotal} 已审核。Holdout ${holdoutUnlocked?"已解锁。":"保持锁定。"}`,{exact:true})).toBeVisible();const holdout=page.getByRole("button",{name:/holdout/i});if(holdoutUnlocked)await expect(holdout).toBeEnabled();else await expect(holdout).toBeDisabled();const goldApi=await page.evaluate(async()=>{const response=await fetch("/api/knowledge/evaluation-reviews?split=development&limit=1&reviewed=false");return{status:response.status,body:await response.json()};});expect(goldApi.status).toBe(200);expect(goldApi.body).toMatchObject({total:goldSummary.body.counts.development.total-goldSummary.body.counts.development.reviewed,reviewed:goldReviewed,holdoutUnlocked});
  ...
  console.log(JSON.stringify({knowledgeReviewCenter:"passed",checks,factReviews,goldReviewed,goldTotal,holdoutUnlocked,mutations:0,externalCalls:0}));
