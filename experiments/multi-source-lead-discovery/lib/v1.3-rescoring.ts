...
const familyPatterns = [
  ...
  /\b(?:wi-?fi|wlan|wireless)\s+(?:access points?|aps?|controllers?|routers?|mesh|hardware|equipment|l(?:o|ö)sungen?)\b|\bmesh wi-?fi\b/i,
  /\b(?:ethernet|poe|managed|smart|network|netzwerk)[- ]?switch(?:es)?\b|\bswitch(?:es)?\s+(?:with\s+)?(?:ethernet|poe)\b/i,
  /\b(?:network|routing|security)\s+gateways?\b|\b(?:business|smb|soho)\s+routers?\b|\bnetwork firewalls?\b/i,
  /\b(?:outdoor wireless|wireless bridges?|point[- ]to[- ]point|richtfunk|funkbr(?:u|ü)cken?)\b/i,
] as const;

const directOverlapPattern = /\b(?:small business|smb|soho|home office|mittelstand|hotel|hospitality|retail branch|branch office|ferienwohnung|gastronomie)\b/i;
const enterpriseOnlyPattern = /\b(?:data cent(?:er|re)|rechenzentrum|carrier[- ]grade|telecom core|campus fabric|cisco nexus|hyperscale|large enterprise)\b/i;
...
export function assessProductUseCaseFit(excerpts: string[]): { level: number; reason: string; familyCount: number } {
  ...
  if (familyCount >= 3) {
    return { level: 5, familyCount, reason: `${familyCount} Cudy-relevant active-networking product families are evidenced.` };
  }
  if (familyCount >= 2) {
