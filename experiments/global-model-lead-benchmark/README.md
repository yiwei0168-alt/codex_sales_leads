...
- 每次运行有 12 次原生搜索动作的目标资源预算、15 分钟硬超时和 8,000 个可见输出 token 硬上限；没有自动重试。供应商对“搜索动作”的定义和限制接口并不一致：Claude/DeepSeek 接受 `max_uses`，Grok 的 `max_turns` 不等于工具调用数，Lingyu 不接受 OpenAI 的 `max_tool_calls`，Kimi 由客户端观察工具回合。因此超过 12 次不会淘汰结果，而是如实记录实际搜索次数、延迟和估算成本，用于单独的资源效率比较。
...
德国第一轮五家正式调用随后完成，均未重试。OpenAI、Kimi、DeepSeek 可评分；Claude 未触发原生搜索而按规则记零；Grok 在 `max_turns=12` 且关闭并行调用后仍产生 21 个独立完成的 `web_search_call`。在查看候选质量前，用户确认将 12 次改为目标资源预算，Grok 第一轮据此恢复为可评分；该修订及其时点保留在脱敏状态报告中。

全部 15 次计划调用现已尝试完成：10 次产生可评分的原生搜索答案，1 次完成但未搜索而记零，4 次为基础设施失败，且没有重试。Kimi、DeepSeek、Grok 各有三次可评分运行；OpenAI 仅第一轮可评分，后两轮服务过载；Claude 第一轮未搜索，后两轮 Lingyu 无可用账号。因此质量榜只能对实际成功搜索的运行评分，通道成功率与未搜索率必须另列，不能混入模型线索质量结论。

Codex 已按冻结规则处理 10 个原生搜索答案：只抽取编号小节、编号候选表格行或独立加粗候选条目，排除搜索计划、正文举例和过程叙述。共得到 39 个候选出现，跨运行去重为 22 家。Kimi 三次虽触发搜索，但最终回答都是高度重复的“继续搜索”过程文本，未形成离散候选，因此三次均抽取为 0 家。Codex 随后仅用独立公开网页完成 22 家预核验；脱敏审核包已生成，最终类别、有效联系人计数和质量分仍等待用户盲审。为准确记录制造商自身或自营 Marketplace 等非独立对象，拒绝原因新增 `not_independent_sales_lead`；它仍是 0 分，不改变评分公式。
