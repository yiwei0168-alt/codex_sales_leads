...
- 每次运行最多允许 12 次原生搜索动作、15 分钟、8,000 个可见输出 token；没有自动重试。限制是防止单次异常运行无限消耗资源的安全上限，不是要求模型必须用满。Claude/DeepSeek 使用供应商 `max_uses`，Kimi 由客户端逐次计数，Grok 关闭并行工具调用后使用 `max_turns`。Lingyu 不接受 OpenAI 的 `max_tool_calls`，因此 OpenAI 只能在响应后计数，超过 12 次的运行记零；这是明确披露的网关限制，而非服务端硬上限。
...
德国第一轮五家正式调用随后完成，均未重试。OpenAI、Kimi、DeepSeek 可评分；Claude 未触发原生搜索而按规则记零；Grok 在 `max_turns=12` 且关闭并行调用后仍产生 21 个独立完成的 `web_search_call`，超过冻结的 12 次动作上限而记零。由于该结果证明 xAI 的轮次限制不等于搜索动作限制，第二、三轮已暂停，等待资源规则调整决定。
