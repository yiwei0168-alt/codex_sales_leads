  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    const cellDeltas = byCell.map((cell) => {
      ...
      for (let slot = 0; slot < slotsPerCell; slot += 1) {
        const index = Math.floor(random() * slotsPerCell);
...
export function calculateExperimentMetrics(inputs: MetricCellInput[], blindAuditPassed: boolean): ExperimentMetricReport {
  ...
  const byCell = inputs.map((input) => {
    const arms = Object.fromEntries((["gemini-native", "product-e2e"] as const).map((arm) => {
      ...
      const values = input.arms[arm].slice(0, slotsPerCell).map((slot) => {
      ...
      const missingSlots = Math.max(0, slotsPerCell - values.length);
      ...
      const metrics: ArmCellMetrics = { slotUtilities, cellUtility: mean(slotUtilities),
        ...
        ndcgAt50: ndcg(slotUtilities), duplicateSlots, missingSlots };
      return [arm, metrics];
  ...
  const countryCodes = EXPERIMENT_CONFIG.markets.map((market) => market.countryCode) as ExperimentCountryCode[];
  const marketDelta = (Object.fromEntries(countryCodes.map((countryCode) => [countryCode,
