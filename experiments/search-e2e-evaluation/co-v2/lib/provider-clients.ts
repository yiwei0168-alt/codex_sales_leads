...
async function geminiControlResponseFormat(): Promise<Record<string, unknown>> {
  const filename = path.resolve("experiments/search-e2e-evaluation/co-v2/schemas/gemini-control-output.schema.json");
...
export async function callClaudeBlindJudge(packet: Record<string, unknown>, model: string,
  ...
  const rubric = await readFile(path.resolve("experiments/search-e2e-evaluation/co-v2/config/blind-judge-rubric-v2.md"), "utf8");
  const schema = JSON.parse(await readFile(path.resolve("experiments/search-e2e-evaluation/co-v2/schemas/blind-judge-output-v2.schema.json"), "utf8")) as Record<string, unknown>;
  const startedAt = new Date().toISOString();
  const started = Date.now();
  let response: Awaited<ReturnType<typeof requestJsonWithRetry>>;
  try {
