...
export async function runControlCell(cell: ExperimentCell, options: {
  ...
  const candidates = (call.output?.candidates ?? []).slice(0, targetCount).map((candidate, index) => ({
    ...candidate,
    rank: index + 1,
  ...
  return { schemaVersion: 1, runId: EXPERIMENT_CONFIG.runId, cellId: cell.cellId, arm: "gemini-native",
    ...
    missingSlots: Math.max(0, targetCount - candidates.length), ...(call.parseError ? { parseError: call.parseError } : {}),
