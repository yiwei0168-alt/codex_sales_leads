...
const frozenTag = "search-e2e-co-v2.0.22-preregistered";
...
const frozenFiles = [
  ...
  "scripts/migrate-blind-cache-keys-v2.ts",
...
async function freezeManifest(): Promise<void> {
  ...
  await writeJsonAtomic(path.join(experimentRoot, "config/frozen-manifest.v2.0.22.json"), {
  ...
  console.log(JSON.stringify({ status: "manifest-frozen", fileCount: files.length,
    manifest: "experiments/search-e2e-evaluation/co-v2/config/frozen-manifest.v2.0.22.json" }, null, 2));
}

async function verifyFrozenManifest(requireTag = true): Promise<void> {
  validateExperimentConfig();
  const manifest = JSON.parse(await readFile(path.join(experimentRoot, "config/frozen-manifest.v2.0.22.json"), "utf8")) as {
...
async function runEvaluation(): Promise<void> {
  await verifyFrozenManifest(true);
  const state = await loadRunState();
  if (!["cells-completed", "evaluation-running", "blind-audit-running", "completed"].includes(state.status)
  ...
  const executed = await executeBlindAuditV2(sample, { judgeModels,
    ...
    previousCostEvents: state.costEvents,
