...
const frozenTag = "search-e2e-co-v2.0.13-preregistered";
...
async function freezeManifest(): Promise<void> {
  ...
  await writeJsonAtomic(path.join(experimentRoot, "config/frozen-manifest.v2.0.13.json"), {
  ...
  console.log(JSON.stringify({ status: "manifest-frozen", fileCount: files.length,
    manifest: "experiments/search-e2e-evaluation/co-v2/config/frozen-manifest.v2.0.13.json" }, null, 2));
}

async function verifyFrozenManifest(requireTag = true): Promise<void> {
  validateExperimentConfig();
  const manifest = JSON.parse(await readFile(path.join(experimentRoot, "config/frozen-manifest.v2.0.13.json"), "utf8")) as {
