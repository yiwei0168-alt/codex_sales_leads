...
import { cellById, experimentCells, EXPERIMENT_CONFIG, leadPlanForCell, validateExperimentConfig,
...
import { artifactRunRoot, loadRunState, rawRunRoot, saveRunState, writeJsonAtomic,
...
const frozenTag = "search-e2e-eval-v1.0.0-frozen";
...
function canadaPlan(category: "distribution" | "resale" | "retail" | "si-msp" = "distribution"): LeadSearchPlan {
  ...
  return { countryCode: "CA", countryName: "Canada", objective: "new-market", roles: [...roles], targetCount: 2,
    queryLanguage: "en", userRequest: `Find 2 ${category} networking companies in Canada`,
...
async function runPreflight(): Promise<void> {
  ...
  for (const [index, call] of (intent.plannerCalls ?? []).entries()) {
    events.push(preflightEvent({ eventId: `preflight:intent:${index + 1}`, runId: state.runId,
  ...
  checks.push({ name: "kimi-intent", ok: true, model: intent.plannerModel, source: intent.plannerSource });
  ...
  for (const [index, usage] of embeddingUsage.entries()) events.push(preflightEvent({
  ...
  checks.push({ name: "local-rag", ok: true, citationCount: rag.length });
  ...
  for (const [key, step] of routeSteps) {
    ...
    events.push(preflightEvent({ eventId: `preflight:discovery:${key}`, runId: state.runId,
    ...
    checks.push({ name: `discovery-${key}`, ok: true, results: result.items.length, retries: result.retryCount });
  }
  ...
  events.push(preflightEvent({ eventId: "preflight:tavily", runId: state.runId, ledger: "product-e2e-arm",
  ...
  checks.push({ name: "tavily-evidence", ok: true, results: tavily.results.length });
  ...
  for (const [index, usage] of scored.usage.entries()) events.push(preflightEvent({
  ...
  checks.push({ name: "deepseek-score-only", ok: true, model: scored.assessments[0].model });
  ...
  events.push(preflightEvent({ eventId: "preflight:gemini-control", runId: state.runId,
  ...
  checks.push({ name: "gemini-3.6-flash-structured-search", ok: true,
  ...
  let blindModel = EXPERIMENT_CONFIG.blindAudit.primaryModel;
  let blind = await callClaudeBlindJudge(blindPacket, blindModel);
  if (!blind.output) {
    events.push(preflightEvent({ eventId: "preflight:blind-primary-invalid", runId: state.runId,
  ...
  events.push(preflightEvent({ eventId: "preflight:blind-judge", runId: state.runId,
    ledger: "evaluation-overhead", arm: "shared-evaluation", stage: "preflight-blind-judge",
    provider: "anthropic", requestedModel: blind.requestedModel, actualModel: blind.actualModel,
    startedAt: blind.startedAt, completedAt: blind.completedAt, latencyMs: blind.latencyMs,
    attempts: blind.attempts, retries: blind.retries, fallbackUsed: blindModel !== EXPERIMENT_CONFIG.blindAudit.primaryModel,
  ...
  checks.push({ name: "claude-blind-judge", ok: true, model: blind.actualModel });

  const unpriced = events.filter((item) => item.budgetCostUsd === null);
  if (unpriced.length > 0) throw new Error(`Preflight has unpriced events: ${unpriced.map((item) => item.eventId).join(", ")}`);
  state.costEvents.push(...events);
  state.blindJudgeModel = blindModel;
  ...
  await writeJsonAtomic(path.join(artifactRunRoot(), "preflight/preflight-report.json"), {
    schemaVersion: 1, runId: state.runId, status: "passed", completedAt: new Date().toISOString(), checks,
    chosenBlindJudgeModel: blindModel, cost: summarizeCostEvents(events), budgetDecision: decision,
  });
  console.log(JSON.stringify({ status: "preflight-passed", blindJudgeModel: blindModel,
    cost: summarizeCostEvents(events), budgetDecision: decision }, null, 2));
}
...
async function runCell(cellId: string): Promise<void> {
  ...
  await saveRunState(state);
  const runners = { "gemini-native": () => runControlCell(cell), "product-e2e": () => runProductCell(cell) };
  ...
  await writeJsonAtomic(path.join(rawRunRoot(), `cells/${cell.cellId}/gemini-native.json`), control);
  await writeJsonAtomic(path.join(rawRunRoot(), `cells/${cell.cellId}/product-e2e.json`), product);
  await writeJsonAtomic(path.join(artifactRunRoot(), `cells/${cell.cellId}/gemini-native.json`), publicControlResult(control));
  await writeJsonAtomic(path.join(artifactRunRoot(), `cells/${cell.cellId}/product-e2e.json`), publicProductResult(product));
  state.costEvents.push(...control.costEvents, ...product.costEvents);
