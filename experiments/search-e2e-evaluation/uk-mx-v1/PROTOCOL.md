# Cudy端到端搜索能力正式测评协议 v1.0.6

状态：`preregistered-pending-preflight`

v1.0.6修复本地解析对否定句的误判：英文、西语和中文中被“不要/排除/No busques”等明确否定的Agent、Brand Owner或OEM/ODM不再被视作显式请求；只有正向明确请求才允许这些特殊角色/机会。v1.0.5调用已保留538个输入token、1211个输出token、47.170秒和0.0049077533美元。v1.0.0至v1.0.5均未运行任何英国/墨西哥正式单元；此前七次Kimi预检合计以0.042232美元（向上取整）计入总预算。既有目标归一化、类别门禁、120秒超时、逐阶段即时落账及断点续跑全部保留。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
