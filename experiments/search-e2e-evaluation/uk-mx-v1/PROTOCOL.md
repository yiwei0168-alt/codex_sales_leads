# Cudy端到端搜索能力正式测评协议 v1.0.4

状态：`preregistered-pending-preflight`

v1.0.4修正过严的意图语义门禁：对于`Distributor/VAD`等同一实验类别，Kimi只需返回至少一个且不得返回类别外角色；国家、数量和目标仍必须完全一致。实际执行计划始终覆盖为冻结单元的完整角色集合，意图模型不能缩减或扩张实验范围。v1.0.3调用已保留538个输入token、1070个输出token、30.503秒和0.0043464890美元。v1.0.0至v1.0.3均未运行任何英国/墨西哥正式单元；此前五次Kimi预检合计以0.030768美元（向上取整）计入总预算。既有Schema归一化、120秒超时、逐阶段即时落账及断点续跑全部保留。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
