# Cudy端到端搜索能力正式测评协议 v1.0.1

状态：`preregistered-pending-preflight`

v1.0.1仅修复正式开始前预检发现的运行可靠性问题：`new_market`确定性归一化为`new-market`，Kimi安全超时由45秒调整为120秒，并将预检及正式单元成本改为逐阶段即时落账、预检检查点续跑和已完成单臂断点续跑。v1.0.0未运行任何英国/墨西哥正式单元；其两次Kimi预检以0.01美元保守调整计入总预算。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
