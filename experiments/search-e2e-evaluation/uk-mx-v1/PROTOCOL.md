# Cudy端到端搜索能力正式测评协议 v1.0.2

状态：`preregistered-pending-preflight`

v1.0.2在v1.0.1可靠性修复基础上补齐Kimi fallback失败调用的usage/延迟/重试保留，并为供应商未返回usage的失败调用设置0.01美元保守预算占位。v1.0.0与v1.0.1均未运行任何英国/墨西哥正式单元；此前三次Kimi预检合计以0.02美元保守调整计入总预算。v1.0.1的枚举归一化、120秒超时、逐阶段即时落账、预检检查点续跑和已完成单臂断点续跑全部保留。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
