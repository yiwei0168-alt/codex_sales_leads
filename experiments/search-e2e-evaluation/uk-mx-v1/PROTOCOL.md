# Cudy端到端搜索能力正式测评协议 v1.0.5

状态：`preregistered-pending-preflight`

v1.0.5将Kimi返回的`objective`先作为受限字符串接收，再通过明确同义映射及本地解析结果归一化为两个产品枚举；无法识别时使用本地解析的目标。正式实验的最终objective仍由冻结单元强制覆盖。v1.0.4调用已保留538个输入token、1625个输出token、40.614秒和0.0065557210美元。v1.0.0至v1.0.4均未运行任何英国/墨西哥正式单元；此前六次Kimi预检合计以0.037324美元（向上取整）计入总预算。既有类别门禁、120秒超时、逐阶段即时落账及断点续跑全部保留。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
