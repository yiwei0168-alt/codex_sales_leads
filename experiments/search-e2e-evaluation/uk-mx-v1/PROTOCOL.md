# Cudy端到端搜索能力正式测评协议 v1.0.3

状态：`preregistered-pending-preflight`

v1.0.3将Kimi返回的非关键`coverage_mode`同义表达确定性归一化；未知表达回落到该字段原本的缺省值`auto`，不改变国家、角色、数量或搜索目标。v1.0.2已成功保留失败调用的538个输入token、1591个输出token和0.0064203807美元价格。v1.0.0至v1.0.2均未运行任何英国/墨西哥正式单元；此前四次Kimi预检合计以0.026421美元（向上取整）计入总预算。既有120秒超时、逐阶段即时落账、预检检查点续跑和已完成单臂断点续跑全部保留。这些修改不改变实验样本、搜索策略、模型、评分或胜负标准。
