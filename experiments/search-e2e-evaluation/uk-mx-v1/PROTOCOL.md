...
> 2026-09-07 next-evaluation blind-audit amendment: v1.1.6 and its conclusion remain immutable. Blind-audit v2.0.0 is active only for the next formal evaluation. Each of the eight cells contributes six deterministic probability-sample cases that are independent of arm, rank, score and scoring citations, plus two diagnostic stress cases. Only the 48 representative cases control pass/fail; the 16 stress cases are reported separately. Two different high-capability provider/model families judge every packet without Web search. A third decision is requested only for a total-score difference of at least eight points, a role-family disagreement, or an identity/market/request-family/score-threshold/eligibility disagreement. The main role gate is role-family agreement; exact subtype is diagnostic. Qualification components, within-cell macro Spearman, citation-ID validity and citation claim entailment are separate metrics. Frozen current-run evidence is selected by a score-independent packet policy and reused without new search or evidence acquisition. Judges cannot see arm, provider, model, rank, existing score, eligibility, sample cohort or stress stratum. See `config/blind-audit-v2.0.0.json`.

> Every valid v2 judge or arbitrator result is checkpointed immediately under a dependency-complete cache key. The key includes protocol version, full packet, judge identity, requested model and—for arbitration—the two upstream outputs. Reruns read this cache before any model call, expose cache hit/miss/write counts and fail closed on mismatched entries or missing score-independent evidence.

> 2026-09-07 post-completion infrastructure note: the completed v1.1.6 outcome and frozen artifacts are unchanged. Future runs persist a minimal candidate/provider attribution projection outside raw payloads, reconstruct it during frozen-arm reuse, and attribute aggregate shared-evaluation stage volume only once across routine/escalation model cost events. The post-run audit remains authoritative for v1.1.6.
