> MA15 变更：采用已确认的半透明紫色视觉、三个业务入口及资料默认页；四阶段已实现，模式/资料隔离、六组布局/路由、真实批准与模拟断线恢复、构建及静态资源通过；验收边界见 MA15_UI_ACCEPTANCE_2026-09-21.md。精确规则与状态见 [规则](docs/CONFIRMED_PRODUCT_RULES.md) 和 [工作流](docs/FRONTEND_SIMPLIFICATION_PLAN_2026-09-21.md)。
...
2026-09-14 阶段133 / B25、B26：条件复核与分歧裁决的已验证结构化响应按实际请求、模型/输出契约及用户/国家身份即时保存，后续失败不重购已完成子调用；写入失败暂停且不丢历史费用。当前 Terra/Sol 费用合同仍阻止真实调用，跨进程 SQL 与 Agent 合成恢复为分别验证，A11 自然语言真实闭环未通过。[验收](docs/REVIEW_SUBCALL_CHECKPOINT_ACCEPTANCE_2026-09-14.md)。

2026-09-13 阶段33 / D13-O04：Brave为四类搜索核心，Places/Exa/Gemini按类别已购贡献条件启用，任务内连续两次成功新查询无新增暂停可选工具。故障/缓存不能伪装成无贡献，任何工具均未全局关闭。跨轨相同请求复用保留真实请求差异，策略/请求契约版本参与恢复依赖。无新实验；788测试与生产构建通过，真实整体验收继续。

2026-09-13 阶段32 / D13-O04：已购搜索输出、已完成门禁与原费用在轮内可恢复；遥测/轮次汇总幂等，不因保存失败重跑模型或误判供应商故障。恢复绑定任务、国家、工作区与请求契约。无新增实验或付费验证；跨轨去重、条件路由与真实产品整体验收继续。

2026-09-13 阶段31 / D13-O04：工具健康及已购发现结果在同任务轮次间复用，依赖身份覆盖原请求、国家、任务与服务商配置。快照不保留明文配置或原始响应包。当前实现以完整发现轮次检查点为边界，单调用中断恢复与类别边际收益条件路由尚待完成。

2026-09-13 阶段30 / D13-O03：最终目标以持久化实际保存数量复核，角色待判不计入合格且不等同市场耗尽。故障不能依据旧无新增计数证明耗尽。任务执行结束与目标达成分开呈现，通知/详情共用停止原因并展示缺口。O02真实SQL隔离/幂等/范围外状态验收通过；真实业务/UI闭环仍未完成。

2026-09-13 阶段29 / D13-O02：错类转移只能进入原请求明确包含的角色，复用已完成证据与校正；公司+国家+目标类别保留可追踪关联，重复公司只进入一次评分队列。范围外保留状态，冲突主角色待判，不扩张用户范围。独立路由检查点防止保存失败重跑模型。真实路由持久化验收继续。

2026-09-13 阶段28 / D13：续搜不永久排除未完成评分，不以连续零结果替代有依据的市场耗尽。处理缺项必须复用原检查点及费用身份，禁止新续搜绕过。真实 PostgreSQL 跨进程检查点和费用恢复验证通过，完整业务闭环尚未完成。

2026-09-13 阶段27 / D13-O01：缺项检查点与已有失败任务恢复入口接通，恢复执行仅使用原证据处理缺失项，完整评分及费用保留。任务级未知费用核验先于恢复节点授权；合法费用核销不会删除历史记录。当前仅合成恢复验证通过，O01 及整体验收仍未完成。

2026-09-13 阶段26 / D13：O01 的异常角色完成状态、缓存门禁、处理未完成停止条件已接入。具体角色必须属于证据支持角色与对应家族；空角色 Hybrid 不能视为有效校正。技术缺项保留结果及费用，不自动重复付费，不以新搜索掩盖缺项。恢复执行及 O02–O05 完整实现/验收仍待完成，冻结测评不变。
...
- 邮箱按文件夹与时间范围同步，默认最近 180 天、每次最多 100 封；明确指定历史起止日期时保留该范围，单次上限不变。复用已同步数据；按地址/域名/会话关联公司，歧义由用户选择。
