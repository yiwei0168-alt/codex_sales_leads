...
- 用户选中候选公司后，Development Strategy Agent 只检索轻量专库中的 Cudy 公司档案、分销政策、市场背书和高价值反馈记忆，并结合脱敏长模板与候选证据，用一次 Kimi-k3 调用生成策略和草稿；每一版都必须审核，批准不会自动发信。
...
7. 在开发助手中由 Kimi-k3 生成策略和草稿，提交优化反馈可生成新版本；Agent 只把可复用的市场事实、渠道策略或稳定风格偏好写入个人记忆。人工确认后批准，批准状态仅为未来受控发信接口预留。
