...
- Kimi-k3 基于最近多轮对话识别意图、拆分执行计划，并在低置信度时继续追问；
- 产品、公司和已审核邮箱知识通过私有 RAG 回答并附引用；
- 同时需要内外部信息的问题并行执行内部 RAG 与 Gemini Google Search，再由 OpenAI（Lingyu 网关）整合；
...
- 真实搜索结果经官网取证、域名去重、独立评分与条件盲审后，仅保存达到 50 分且必要复核已完成的公司；复核失败或要求补证不会填充结果数。
- 用户选中候选公司后，Development Strategy Agent 只检索轻量专库中的 Cudy 公司档案、分销政策、市场背书和高价值反馈记忆，并结合脱敏长模板与候选证据，用一次 Kimi-k3 调用生成策略和初稿；人工反馈后的重写与私人记忆筛选使用 Claude。每一版都必须审核，批准不会自动发信。

早期墨西哥验证数据和脚本保留为回归资料，但不再限制产品运行市场。搜索候选在完成网页证据复核前统一标记为 `Inferred`。

## 当前验收状态（2026-09-15）
- 阶段 248 已按 A34 将知识库检索后的回答模型从 OpenRouter Sol/Bedrock 切换为 Moonshot 直连 `kimi-k3`，单次 JSON 回答、4,096 completion 上限、无模型备用或自动重试，并保留 A30 公开来源脱敏与 A33 费用观察。一次真实公开合成验收 HTTP 200，387 prompt tokens、340 completion tokens、11,152 ms、引用有效；现金费用未由响应报告，保持未知。详见[阶段 248 证据](docs/A34_KIMI_RAG_ANSWER_STAGE248_2026-09-15.md)。
- 阶段 247 已按 A33 将当前精确用户切换到付费调用“观察模式”：调用前不执行费率、预算、冻结或费率暂停门禁，不预留金额；调用后记录可得的实际费用、token、延迟和利用率，缺失金额明确标为未知。认证、租户/隐私边界与并发/成功结果防重复仍保留。RAG 在该用户遇到 OpenRouter 边缘 401/403/429/5xx 时可继续尝试一次既定 Bedrock 路由；外部网关仍可能自行拒绝。详见[阶段 247 证据](docs/A33_OWNER_PAID_OBSERVATION_MODE_STAGE247_2026-09-15.md)。
- 阶段 247 的后续最小路由诊断已精确定位当前 RAG 失败：OpenAI 与 Bedrock 两条同模型请求均由 OpenRouter 在 `routerAttempt=0` 返回 403 `This model is not available in your region.`；这不是本地费用门禁、密钥失效或用户自设 Guardrail。阶段 248 已改用 Kimi，并同步把 401 认证失败与 403 地域/权限拒绝拆分显示。
- 阶段 246 已按 A32 将 RAG 合同放宽为一次受控同模型备用：OpenAI Sol 主线路 USD0.778240，只有路由元数据证明上游 401/403/429/5xx 时才调用 `amazon-bedrock/us-east-1`，备用 USD0.378471，合计最坏预留 USD1.156711；输出降至 4,096，SDK/网关隐藏回退均关闭，失败后无第三次调用。A30 公开来源与脱敏边界不变，并新增脱敏状态/供应商/哈希请求 ID 观测。公开费率、SDK 无网络回退、真实 SQL 双合同、1,077 项全量测试和生产构建通过，0 次真实推理；真实语义和账单仍待验收。详见[阶段 246 证据](docs/A32_CONTROLLED_RAG_FALLBACK_STAGE246_2026-09-15.md)。

- 阶段 245 已按 A31 为知识库 RAG 回答建立独立合同：OpenAI-only Sol、61,440-byte 输入、8,192 `max_tokens`、单次保守预留 USD0.901120，不再复用 USD27.345252 全上下文合同。按 A30 只把明确公开来源且经过敏感模式过滤的知识副本发送给外部模型；内部培训、邮箱和无公开来源标记的资料保持本地。公开端点、实际 SDK 无网络线形、全量回归及真实 SQL 合成预留/防重复验证通过，0 次真实推理。详见[阶段 245 证据](docs/A31_RAG_ANSWER_CONTRACT_STAGE245_2026-09-15.md)。
- 阶段 244 已按 A30 允许公开／公开来源业务信息外发，同时禁止 API 密钥、个人信息等敏感内容进入模型请求体。Terra 复核包新增外发副本脱敏，16 项定向测试和 TypeScript 通过；同一检查点实际发送 2 次经检查的 v2 请求，但仍为 0 有效输出、费用/token 未知，新增保守占用 USD22.038404，累计 USD83.947114。任务保持失败、0 最终公司，不再盲目重放。详见[阶段 244 证据](docs/A30_PUBLIC_EXTERNAL_DISCLOSURE_STAGE244_2026-09-15.md)。
- 阶段 243 已按 A29 启用当前用户临时付费准入。原检查点恢复实际发出 2 次 Terra v1 重放，均无有效输出且费用未知，保守占用增加 USD22.038404、累计为 USD61.908710。当前端点元数据定位旧请求含不受支持的 `temperature`；Terra-only v2 改用 `max_tokens=8192` 并通过本地/SQL 验证，后续真实结果见阶段 244。详见[阶段 243 证据](docs/A29_PAID_REPLAY_ADMISSION_STAGE243_2026-09-15.md)。
...
- 当前执行以[阶段 242 分层质量与失败进度证据](docs/LAYERED_QUALITY_AND_FAILURE_PROGRESS_STAGE242_2026-09-15.md)、[阶段 241 受控正结果闭环](docs/CONTROLLED_ACTUAL_REVIEW_PRODUCT_LOOP_STAGE241_2026-09-15.md)和[当前唯一验收矩阵](docs/CURRENT_ACCEPTANCE_MATRIX_2026-09-13.md)为准。受控产品层已完成；真实 Terra 语义、完整供应商账单/现金成本与原整体验收仍单列未通过。
...
1. 在首页直接询问产品、公司、认证或已审核邮箱知识；可以在后续消息中纠正、补充或反馈。
...
7. 在开发助手中由 Kimi-k3 生成策略和初稿，提交优化反馈后由 Claude 生成新版本；Agent 只把明确授权且可复用的市场事实、渠道策略、发件人身份或稳定风格偏好写入私人记忆。人工确认后批准，批准状态仅为未来受控发信接口预留。
...
临时 A33 付费观察模式默认关闭。仅可在启动进程中同时设置 `PAID_CALL_STAGE_OVERRIDE=A33` 与 `PAID_CALL_STAGE_OVERRIDE_USER_ID=<精确用户 UUID>`；它只为该用户取消调用前的费率/预算门禁与金额预留，调用后仍记录实际或未知费用，阶段结束后应移除。认证、敏感信息边界和防并发/成功结果重复不受影响。
...
- [当前产品架构（飞书发布版）](docs/CURRENT_ARCHITECTURE_FEISHU.md)
